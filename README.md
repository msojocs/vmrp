...
当前代码仍保留上游的兼容标识：CMake 目标与桌面可执行文件名为 `vmrp`，共享库名为 `libskyengine`，公共 C API 使用 `vmrp_api_*` 前缀，环境变量使用 `VMRP_*` 前缀。这些名称属于现有构建和 ABI 接口，不代表对外项目名称；在完成兼容迁移前不会仅为品牌改名而破坏已有集成。
...
产物名称为 `libskyengine.so` 或 `vmrp.dll`。公共 ABI 位于 [`src/include/vmrp_api.h`](src/include/vmrp_api.h)，主要包括：
