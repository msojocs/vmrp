...
`build/skyengine build/mythroad/gtcm.mrp --screen 320x480`（真机竖屏面板规格）：
...
时显式 `-UVMRP`（CMakeLists.txt:292），不能访问 `skyengine_config` —— 首版实现
...
2. `src/skyengine.c` + `src/include/skyengine.h`
   - `skyengine_display_width/height()`：奇数旋转返回面板转置，否则面板尺寸。
     `skyengine_config.screen_width/height` 始终是 `--screen` 面板尺寸不变。
...
4. `src/main.c`（SDL 展示）
   ...
   - `guiDrawBitmap` wrapper 行宽 → `skyengine_display_width()`。
   - PPM dump / e2e 帧捕获 / SCREEN 命令取 surface->w/h，自动正确。
5. `src/skyengine_api.c` + `skyengine_api.h`（Flutter 共享库）
   - 裁剪/行宽按显示尺寸；`skyengine_api_get_screen_width/height` 返回显示尺寸；
   - 新导出 `skyengine_api_get_screen_rotation()`（0..3，轮询风格与
     get_screen_dirty 一致）；`skyengine_api_init` 归零旋转;
...
- [x] 共享库 `-DSKYENGINE_BUILD_SHARED=ON` 构建通过，`skyengine_api_get_screen_rotation`
      已导出（nm 验证）
