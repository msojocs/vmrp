...
- `ctest --test-dir build --output-on-failure`：通过，`1/1 skyengine_unit_tests`
...
- `src/main.c:137-165` 的 `guiDrawBitmap()` 按 `bmp[xx + yy * skyengine_config.screen_width]` 读取源像素。
- `src/skyengine_api.c:19-32` 的 FFI 版本使用同样的整屏索引。
...
- `src/skyengine_api.c:119-129` 的 `skyengine_api_destroy()` 只释放 `screen_buf` 和编辑文本。
- `src/skyengine.c:360-397` 的 `startVmrp()` 会调用 `skyengine_runtime_init()`，成功路径不调用 `skyengine_runtime_destroy()`。
- `src/runtime_native_mythroad.c:39-42` 的 `skyengine_runtime_destroy()` 才会调用 `native_dsm_funcs_destroy()`。
...
FFI 宿主调用 `skyengine_api_init()` / `skyengine_api_start()` / `skyengine_api_destroy()` 后，DSM 内存、音频设备和内部 Mythroad 状态不会被完整释放。重复启动不同 MRP 时，状态泄漏和资源泄漏风险较高。
...
- `src/fileLib.c:223-236` 的 `skyengine_writeFile()` 不检查 `my_open()` 返回 0，就继续 `my_write()` / `my_close()`。
...
- `src/skyengine.c:149-157` 的 `--screen` 路径会校验正数。
- `src/skyengine.c:184-187` 的 `SKYENGINE_SCREEN_WIDTH` / `SKYENGINE_SCREEN_HEIGHT` 使用 `atoi()`，不校验正数、上限或溢出。
- `src/skyengine_api.c:78-86` 的 `skyengine_api_init()` 对非正数回退默认值，但没有限制过大值，且 `(screen_w * screen_h)` 先以 `int` 相乘再转 `size_t`。
- `src/skyengine_api.c:92-93` 在 API init 中调用 `prepareVmrpArgs(1, fake_argv)`，环境变量可以覆盖调用者传入的分辨率。
- `docs/flutter-integration.md:728` 写明分辨率通过 `skyengine_api_init(width, height)` 设置。
...
- `CMakeLists.txt:294-303` 的 `test_skyengine` 只编译 test 文件、`test/stubs.c` 和 Mythroad 源码。
...
- `src/skyengine_api.c:182-185` 的 `skyengine_api_set_edit_text()` 也没有检查 `my_mallocExt()` 返回值。
