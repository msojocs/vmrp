...
VMRP_ARM_EXT_DIAG=1 \
./build/skyengine --work-dir . test/fixtures/example.mrp
```
...
桌面程序可通过 `SKYENGINE_PPM_PATH` 指定截图路径。设置 `VMRP_PPM=1` 后，每次有效提交会更新最新 PPM：

```bash
VMRP_PPM=1 \
SKYENGINE_PPM_PATH=/tmp/skyengine-screen.ppm \
./build/skyengine --work-dir . /path/to/app.mrp
```
...
```bash
VMRP_ARM_EXT_SMOKE=mrc/asm/asm.mrp ./build/skyengine --work-dir .
```
...
1. `cmake --build build --target skyengine` 通过。
2. `git diff --check` 通过。
3. 相关真实 MRP 用例通过。
