...
| SCRRAM | `0x50000000` | 16 MB | `MR_MALLOC_SCRRAM` 独立扩展内存带 |
...
`table[38]` 的 `MR_MALLOC_SCRRAM`（1014）不是 `LG_mem` 分配。真机平台从
独立扩展内存返回可供图像、Flash 和离屏缓冲使用的地址；执行器将其惰性映射到
`EXT_SCRRAM_ADDR`。该区域不改变 `table[108..111]` 的主堆边界或统计，也不占用
主映射内的 bump、wrapper 栈和 wrapper code 容量。超过 16 MB 的单次请求明确失败。
