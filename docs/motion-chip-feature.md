...
1. **DSM 层状态机**（src/mythroad/dsm.c + dsm.h，真机上这是传感器驱动状
   态；注意 mythroad 源码 `-UVMRP` 编译，不能访问 skyengine_config）：
...
3. **注入链**：`mr_motion_input(x,y,z)`（mythroad.c）查监听状态→写槽位→
   `mr_event(18, mode, slot)`；`skyengine_runtime_motion`（runtime 层）→
   `skyengine_motion_input`（skyengine.c/skyengine.h，宿主入口）。
...
5. **Flutter API**（skyengine_api.c/h）：`skyengine_api_motion(x,y,z)`（异步队列
   API_CMD_MOTION）、`skyengine_api_motion_active()`（-1/0/1，嵌入端据此开关平
...
- [x] 全量回归 25 文件 47 用例通过。⚠️ 教训：**跑全量 e2e 时不要并行运行
      手动 skyengine 探测进程**——曾因资源竞争造成 11 文件假失败
- [x] 共享库导出 `skyengine_api_motion`/`skyengine_api_motion_active`（nm 验证）
