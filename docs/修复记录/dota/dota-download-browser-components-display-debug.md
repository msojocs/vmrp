...
- Preserved directories: `/tmp/skyengine-e2e-7VsswN` and `/tmp/skyengine-e2e-pfMs7A`.
...
- Preserved directory: `/tmp/skyengine-e2e-NMVZag`.
...
- Narrow diagnostics with `/tmp/skyengine-e2e-PzxxLk` showed no `UC_ERR`/crash; the browser component download path continued issuing `simpleDownload` requests. Screen diagnostics showed accepted full-screen presents from the guest, confirming the horizontal line comes from guest startup frames.
...
- `strings`/offset evidence from `/tmp/skyengine-frame.ext`:
...
- Build verification: `cmake --build build --target skyengine -j2` passed.
...
- Kept verification directory: `/tmp/skyengine-e2e-dbtcUH`.
- PPM evidence:
...
- A custom frame sampler preserved `/tmp/skyengine-e2e-sample-xUy7bc`. After confirming the download result, captures from `250ms` through `12000ms` all remain the same 2-color screen: black background plus `#2844a0` rows `190..191`, bbox `52,190..187,191`. At `14000ms` the screen changes to a 27-color later browser page.
...
- Narrow diagnostics around `/tmp/skyengine-e2e-eo4lmx/stdout.log` show `frame.ext` (`P=0x3CA7CC H=0x2C92A5`) is loaded and initially claims screen writes; its full-screen `table[29]` present is accepted at LR `0x2C9471`, but visible samples are black, matching the 2-color PPM.
...
- Rebuilt with `cmake --build build --target skyengine -j2`; target was already up to date.
...
- Kept directory: `/tmp/skyengine-e2e-KazAU3`.
...
- Verification:
  ...
  - `VMRP_E2E_KEEP_TMP=1 pnpm vitest run test/e2e/dota/download-plugin.test.ts -t "下载浏览器插件 - 成功" --reporter=verbose` passed in `42.77s`; kept directory `/tmp/skyengine-e2e-hPO7o8`.
  - `/tmp/skyengine-e2e-hPO7o8/download-browser-components.ppm`: `240x320`, `unique=1395`, `nonzero=7905`, bbox `52,71..187,234`, `(95,88)=(248,248,120)`, `(120,190)=(40,68,160)`, `(152,146)=(0,16,32)`. The converted PNG shows the browser logo, `冒泡浏览器`, progress line, and `正在更新`.
