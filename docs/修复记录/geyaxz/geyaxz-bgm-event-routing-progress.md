...
- 首次修复运行：`/tmp/skyengine-e2e-xONfhU/`，目标用例 5.585 秒通过；窄日志为
...
- 最终定点运行：`/tmp/skyengine-e2e-cEkkIw/`，目标用例 5.561 秒通过。
- 事件修复前首先记录
...
- `cmake --build build --target skyengine --parallel 2`：通过；
...
- 重新构建后目标用例通过（5.647s，`/tmp/skyengine-e2e-0X45f7`），确认工作区
  修复有效，作为复审基线。
- 复审焦点拆成两条独立证据链：
  1. `mythroad.c` 事件路由：对照仓库根 `mythroad/` 原版平台源码与
     ...
     事件发给 ext，则 20-byte native 路径本是 skyengine 兼容层，改动成立）。
...
- 结论:**宿主侧结构性收缩(arm_ext_restore_drained_event_layer)是
  正确层级的修复**——没有可归责于 skyengine 的缺失调用或时序错误可以更根本地
...
- **mythroad.c 改动获原版源码背书**:初始提交 d5ba4af 的原版 mr_event
  ...
  C ext;20-byte native_ext_event 是 skyengine 为 EXT-only runtime 加的兼容
  入口。改动使 FULL runtime 回归原生单入口语义,方向正确,保留。
- 复审后验证:目标用例通过(5.557s,`/tmp/skyengine-e2e-EQOspp`),最终 PPM
