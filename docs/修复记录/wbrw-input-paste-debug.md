...
- Baseline artifact: `/tmp/skyengine-e2e-gt4MbE` from `VMRP_E2E_KEEP_TMP=1 pnpm vitest run test/e2e/wbrw/input-text.test.ts --reporter=verbose`.
...
- Root-cause evidence from source and disassembly:
  ...
  - `objdump -d -S build/skyengine` maps `editGetText` to the log site and confirms the current binary had the source-level `alloc_string` path before the fix.
...
- Built target: `cmake --build build --target skyengine -j2` passed.
- Focused regression: `VMRP_E2E_KEEP_TMP=1 pnpm vitest run test/e2e/wbrw/input-text.test.ts --reporter=verbose` passed.
- Final verification artifact: `/tmp/skyengine-e2e-4w5oYJ`.
- Runtime log evidence: `/tmp/skyengine-e2e-4w5oYJ/stderr.log` includes `INFO: editGetText(): 'http://wap.baidu.com'`.
- PPM evidence:
...
- Post-build disassembly evidence:
  - `objdump -d -S build/skyengine` shows table[77] now calls `mr_editGetText`, then `wstrlen`, adds `2`, and calls `alloc_bytes` instead of the previous `alloc_string`/`strlen` path.
