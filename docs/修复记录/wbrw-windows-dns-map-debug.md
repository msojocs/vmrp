...
- `vmrp-shared` 和 Windows 原生 `vmrp` 目标显式启用 `NETWORK_SUPPORT`。此前 MSVC 构建没有该宏，`network.c` 的 socket/DNS API 会直接返回失败，导致 DNS 映射无机会生效。
- `network.c` 增加 Windows `CreateThread` 异步 worker 封装，保持 `mr_connect` / `mr_getHostByName` 的 `MR_WAITING` 回调语义，不把异步路径改成同步路径。
- 新增 `VMRP_NETWORK_LOG=1` 或 `VMRP_LOG=1` 时的关键日志：DNS map 原始字符串、解析后的表项、命中/未命中、`getaddrinfo` 目标和最终 `connect` 目标。
...
Windows CLI 冒泡浏览器 smoke 已通过。使用 `build\Debug\vmrp.exe` 启动
`test\fixtures\wbrw.mrp`，显式传入线上 DNS map，日志确认：

```text
[vmrp-net] dns_map[3]: proxy2.51mrp.com -> 159.75.119.124
[vmrp-net] dns_lookup hit: proxy2.51mrp.com -> 159.75.119.124
[vmrp-net] get_host result name='proxy2.51mrp.com' ip=159.75.119.124/0x9F4B777C
[vmrp-net] connect fd=540 target=159.75.119.124:80 ip=0x9F4B777C
[vmrp-net] connect success ip=0x9F4B777C
```
...
```powershell
& 'D:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe' --build build --target vmrp --config Debug
& 'D:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe' --build build\windows\x64 --target vmrp-shared --config Debug
