...
`build/skyengine build/mythroad/wxmdld.mrp` 的 99% 崩溃由两个通用 ARM EXT
兼容问题叠加造成，现均已修复：
...
`/tmp/skyengine-wxmdld-fixed-e2e-5QhMYB`，没有预置 `sky.bmp` 或 `avm_cache`。
关键运行证据：
...
`/tmp/skyengine-wxmdld-final-ot2skl` 重复冷启动；它生成完全相同大小和 SHA-256 的
...
```bash
build/skyengine build/mythroad/wxmdld.mrp
```
...
- `ctest --test-dir build --output-on-failure`：历史 build-tree 的
  `skyengine-unit` 通过，但当前分支已从 CMake/源码树移除该测试，不能把它当成
  当前源码重新构建的单测证据；
...
- `/tmp/skyengine-wxmdld-parserbranch-jBYJZQ/parser-branch.log`；
- `/tmp/skyengine-wxmdld-seq-a4GWJo/sequence.log`；
- `/tmp/skyengine-wxmdld-sourcefield-OEbny0/source-field.log`；
