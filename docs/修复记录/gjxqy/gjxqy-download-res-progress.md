...
## 2026-07-23 temp.test.ts 下载用例复核

本次目标命令是：

```text
pnpm vitest run test/e2e/gjxqy/temp.test.ts -t download --reporter=verbose --retry=0
```

基线失败位置是 `temp.test.ts` 的瞬态 `downloading` 截图断言：本地服务器响应较快时，
截图已经进入三色失败 overlay，`pixel(152,222)` 为黑色而不是场景色。该断言不能证明
下载成功或失败；后续红色像素 `(114,147)=(248,0,0)` 反而是“下载资源失败”的提示点。

重新只读复核包内证据：`temp/gjxqy-res/res_1.mrp` 长度为 273877，SHA-256 为
`ead50bb5986a3f53db8ee1c08c213242e45b21acb05e11be0e2536914da630b1`。解析 MRP 索引后，
`fileid.bin` 位于 offset 63636、packed length 24，gzip 解压后是 `00000fa1`，按大端为
4001。该原始资源包保持不改。

重新解出 `test/fixtures/gjxqy.mrp` 的 `cfunction.ext`，长度 393840，SHA-256 为
`328fdf9361fbbb4b2720479e863be166de74e1c7742e349df0936d94ed8462aa`。全文只出现一次小端
`3c980f00`，位置仍是 `cfunction.ext+0x37B74`，即 slot 1 注册的 `0x000F983C`；未出现
4001 的任一字节序。结合前述 `+0x3B6C..+0x3B92` 直接按大端读取 `fileid.bin` 并比较的
反汇编，当前失败仍收敛为 fixture 配套 ID 不一致，不是 TCP/HTTP 收包 BUG。

按正向安装验收要求，使用已有通用工具生成独立派生 fixture：
...
派生包 SHA-256 为 `94ebbc1cb0b2c0dea74abe0bcf8b656b9ba3a5eb3d10c83b08c3ae1232fe9448`。
服务器只发送所选 MRP 字节并回显请求资源 ID，不动态改写包内容、不做应用特判、不增加
fallback。`temp.test.ts` 的下载用例改为自行启动 `tool/jxqy-server.js`，等待严格服务端
事务完成，再断言安装产物 `res.bin1` 长度 1294042、SHA-256
`11c0543e9e213f635b59ae9dee989078bac87f94257c306a3dc5a34f16abba07`，以及 `res.v` 更新为
`f0030000010000008d9b5fa4`，最后用正向 PPM 场景点替代旧失败色点。

修复后验证：

```text
pnpm vitest run test/e2e/gjxqy/temp.test.ts -t download --reporter=verbose --retry=0
# 1 passed, 1 skipped, 20.84s

pnpm vitest run test/e2e/gjxqy/temp.test.ts --reporter=verbose --retry=0
# 2 passed, 45.89s

pnpm vitest run test/e2e --reporter=verbose --retry=0
# 30 files passed, 53 tests passed, 326.06s
```

附加检查 `pnpm exec tsc --noEmit`、`node --check tool/jxqy-server.js`、
`node --check tool/replace-mrp-entry.js` 和 `git diff --check` 均通过。全量 `test/e2e`
默认不包含 `temp.test.ts`，因此目标用例和 GJXQY 全文件已单独运行。
