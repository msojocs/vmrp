...
- 现象：`build/vmrp mythroad/mpc.mrp` 启动后，从主界面"软左 → 退出 → 确定"三连点（坐标 `(23,305)/(30,280)/(9,310)`）应当让 app 自行退出，**实际不退出**，进程一直挂着直到外部 timeout。
...
- 闭环完全发生在 mpc.mrp 的字节码空间内（19KB ↔ 157KB），整条递归期间 `table[31]/[32]/[54]` 一次都没被调到，所以靠"改 vmrp 的 loader 顺序 / SDK 同步性"修不了。
...
也就是说"先加载 cfunction.ext（332KB）再被 case 800 覆盖"的那个假设的前提条件，在 mpc.mrp 上**根本不成立**——mpc.mrp 直接把它自己的 19KB 引导模块当 primary 装上。在 native 编译里 `loadCode()` 只在非 native VM 编译路径下使用，所以 native vmrp 启动时本来就没动 cfunction.ext。
...
- `/tmp/vmrp_ext_code.bin`：19KB primary（0x80000 区段在 entry 命中时的内存快照）
- `/tmp/vmrp_nested_code.bin`：157KB nested（0x646380 区段）

> 提示：每次 vmrp 重新跑会重新 alloc，绝对地址（0x702C74、0x749860 这类堆地址）会变；`0x80000+`、`0x646380` 的 VMA 是稳定的（受 EXT_BASE_ADDR / EXT_HEAP_ADDR / arm_alloc 顺序决定）。
...
证明这条递归**完全发生在 mpc.mrp 二进制内部**（19KB ↔ 157KB），不经过 vmrp 提供的任何 SDK 入口。因此：

- 修 vmrp 的 timer slot、改 `mr_timerStart` 的同步性，**接不到链路上**——因为根本没去那里。
- 改 vmrp 的 `case 800/802` 加载顺序、改 `native_ext_reset`，**也接不到**——实测 mpc 只调一次 `case 800`，没有"被覆盖的 cfunction.ext"。
...
### 路线 B：vmrp 拦截 0x843C4 一次性返回

在 `arm_ext_load` 加 hook 把 PC 命中 `0x843C4` 的瞬间直接接管：
- 不让它去调 veneer（避免递归），改为把 F 已经写到链表头的那条目通过 vmrp 的 native `mr_timerStart`（table[31]）真正登记一次后立即 `bx lr` 返回。
...
2. 把这些字段映射回 vmrp 的 `mr_timerStart(t)`（直接调 native `mr_timerStart`，host 端拿 SDL_AddTimer 调度）；
...
- 需要先确认 `mr_c_function_P.field_92.field_8.field_0` 这个 state 字段在 native vmrp 下的值。F 早退依赖它 == 3 或 4，目前看到的递归没触发早退，可能就是因为这个字段在 vmrp 下永远是 0。
...
arm-none-eabi-objdump -D -b binary -m armv7 -EL --adjust-vma=0x80000 \
    -M force-thumb /tmp/vmrp_ext_code.bin | less

# 157KB nested（ARM）
arm-none-eabi-objdump -D -b binary -m arm -EL --adjust-vma=0x646380 \
    /tmp/vmrp_nested_code.bin | less
```
...
| VMA | 偏移 in dump | 说明 |
|---|---|---|
| `0x84298` | `0x4298`（vmrp_ext_code.bin） | F 入口 |
...
| `0x662B5C` | `0x1C7DC`（vmrp_nested_code.bin） | nested ext veneer，blx ip = 0x838B4（r1=0 路径） |
...
是 cfunction.ext，分析汇编逻辑看能不能参考"——意思是：vmrp 是 OS 角色，不应该
...
### 9.4 现状：那段 0x664678 在 vmrp 退出 click 里没被调到
...
也就是 mpc 自带的 RESTART-写入路径在 vmrp 这边整个 click 流程里都没被走通。
...
1. **重点排查 click 派发链路**：不要再去 patch mpc 二进制。要找的是为什么在
   vmrp 下 click → softkey 之后到达不了 `0x664678` 这条 RESTART 通路。可挂的
   探针：
