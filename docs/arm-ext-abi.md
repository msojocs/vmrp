...
**初始容量**：`init_table` 按 `--memory`/`SKYENGINE_MEMORY` 从 bump 区分配一块连续区间，默认 1 MB，可配 1–16 MB，当前 ARM 布局将池长钳制到 8 MB。该初始区间缓存为 `origin_mem_addr..origin_mem_addr+origin_mem_len`，但缓存值不是 table bridge 运行时的最终 ABI 边界。
