...
**容量**：512 KB，由 `init_table` 初始化时从 bump 区分配一块连续区间作为 `origin_mem_addr..origin_mem_addr+origin_mem_len`。

**头节点**：`table[146]` 直接暴露 `&LG_mem_free`（头节点地址），存入 `m->origin_mem_head_addr`（宿主侧）和 ARM 可见槽。头节点格式与空闲节点相同：`{next_off, len}`（8 字节）。
...
**统计 slots**：宿主通过 `origin_mem_left_slot`/`origin_mem_min_slot`/`origin_mem_top_slot` 将 `origin_mem_left/min/top` 同步回 ARM 可见变量（`sync_origin_mem_slots`），保证 ARM 代码读到的内存统计与宿主账本一致。
