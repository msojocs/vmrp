...
#include "mmi_rp_vapp_iphoneui_calllog_def.h"
#include "mmi_rp_srv_nw_name_def.h"
#endif/*__CUSTOM_IPHONE_STYLE_CALLLOG__*/
/***************************************************************************** 
 * Define
 *****************************************************************************/
typedef struct
 ...
 * Function
 *****************************************************************************/
static MMI_ID vapp_clog_launch_from_phb(VappCLogList::CLogListIDEnum listId)
{
...
VappCLogList::VappCLogList(CLogPageTypeEnum pageType,
                 VfxBool b2ndReady,
                 CLogListIDEnum listID, 
                 VfxU16 store_idx,
                 mmi_sim_enum sim_id
                 ) :
    m_CLogList(NULL),
...
void VappCLogList::onInit()
{
    ...
    if (m_currPageType == VAPP_CLOG_PAGE_BASIC)
    {
        ...
        m_CLogList->setAlignParent(
            ...
            VFX_FRAME_ALIGNER_MODE_SIDE);
#if defined(__CUSTOM_IPHONE_STYLE_CALLLOG__)
        m_CLogList->m_signalItemTapped.connect(this, &VappCLogList::onListCmdBtnClick);
        m_CLogList->m_signalItemLongTapped.connect(this, &VappCLogList::onItemLongTapped);
        m_CLogList->m_signalCmdButtonClicked.connect(this, &VappCLogList::onItemShortTapped);
        m_CLogList->m_signalItemSelectionStateChanged.connect(this, &VappCLogList::onSelectionChanged);		
#else
        ...
        m_currListID = (CLogListIDEnum)listId;
...
//        m_CLogList->setMenuControlMode(VCP_LIST_MENU_CONTROL_MODE_NORMAL, VFX_FALSE);       
    ...
    else
    {
        ...
        m_button->setText(VFX_WSTR_RES(STR_GLOBAL_CLEAR));
...
void VappCLogList::setListID(CLogListIDEnum listID)
{
    m_currListID = listID;
...
extern "C" int HcCall_hist_populate_call_list(char * city_name,int city_name_size, char *city_number);
#endif
        ...
        )
{
	...
	if (fieldType == VCP_LIST_MENU_FIELD_TEXT)
	{
		...
		if (name != NULL)
		{
			...
			VfxWChar str_buff[MAX_SUBMENU_CHARACTERS + 1];
			
			memset(city_array,0,MAX_SUBMENU_CHARACTERS);
			kal_wstrncpy(str_buff, name, MAX_SUBMENU_CHARACTERS);
			
			mmi_ucs2_to_asc((char *)city_number,(char *)calllog->cid);
			...
			text = VFX_WSTR_MEM((VfxWChar *)str_buff);
		}
#endif/*__MMI_DSM_NEW__*/
#if defined(__CUSTOM_IPHONE_STYLE_CALLLOG__)
	if(calllog->log_type == SRV_CLOG_CLT_MISSED && calllog->is_new_event == VFX_TRUE)
		color = VCP_LIST_MENU_TEXT_RED_COLOR;		
	else
...
#if defined(__CUSTOM_IPHONE_STYLE_CALLLOG__)
		color = VCP_LIST_MENU_TEXT_BLUE_COLOR;
#endif/*__CUSTOM_IPHONE_STYLE_CALLLOG__*/				
	}
#if defined(__CUSTOM_IPHONE_STYLE_CALLLOG__)	&& defined(__IPHONE_STYLE_LISTMENU__)
	else if (fieldType == VCP_LIST_MENU_FIELD_CMD_BUT_EX_TEXT)
	{		
               		text = VFX_WSTR_RES(STR_GLOBAL_DELETE);
	}
#endif		
	else
	{
	...
	switch(calllog->sim_id)
	{
	   case MMI_SIM1:
	            switch (calllog->log_type)
	            {
						...
						image.setResId(IMG_ID_CALLLOG_SIM1_MISSCALL);
						break;

					case SRV_CLOG_CLT_DIALED:
						image.setResId(IMG_ID_CALLLOG_SIM1_OUTCALL);
						break;
								
					case SRV_CLOG_CLT_RECVED:
						image.setResId(IMG_ID_CALLLOG_SIM1_INCALL);
						break;
	   ...
	   case MMI_SIM2:
	            ...
	            {
	                			case SRV_CLOG_CLT_MISSED:
						image.setResId(IMG_ID_CALLLOG_SIM2_MISSCALL);
						break;

					case SRV_CLOG_CLT_DIALED:
						image.setResId(IMG_ID_CALLLOG_SIM2_OUTCALL);
						break;
								
					case SRV_CLOG_CLT_RECVED:
						image.setResId(IMG_ID_CALLLOG_SIM2_INCALL);
						break;
	   ...
	   if(GetClogListStyle() ==VCP_LIST_MENU_MODE_NORMAL)		
            	image.setResId(IMG_ID_CALLLOG_ARROW_N);	
	   else	
	   	image.setResId(VCP_IMG_LIST_MENU_CMD_BUTTON_DELETE_N);			
            return VFX_TRUE;

        case VCP_LIST_MENU_FIELD_CMD_BUT_IMG_HIGHLIGHT:
        case VCP_LIST_MENU_FIELD_CMD_BUT_IMG_PRESSED:
	   if(GetClogListStyle() ==VCP_LIST_MENU_MODE_NORMAL)		
            	image.setResId(IMG_ID_CALLLOG_ARROW_P);	   	
	   else	
	   	image.setResId(VCP_IMG_LIST_MENU_CMD_BUTTON_DELETE_P);	
            return VFX_TRUE;
                            ...
                            )
{
    ...
    if (m_currPageType != VAPP_CLOG_PAGE_BASIC || m_currListID == VAPP_CLOG_LIST_ID_ALL_CALL)
        text = VFX_WSTR_RES(STR_ID_VAPP_CLOG_NO_CALL_LOG);
    else
        text = VFX_WSTR_RES(STR_ID_VAPP_CLOG_NO_MISSED_CALL);
...
#if defined(__CUSTOM_IPHONE_STYLE_CALLLOG__)&&defined(__MMI_DUAL_SIM__)
									 ...
									 VfxU32   index)
{
	...
	if(GetClgListMode() == EDIT_MODE_CLG)
	{
		return;
	}
	...
	srv_mode_switch_get_network_mode(MMI_SIM2) == SRV_MODE_SWITCH_ON
	&&2 == srv_sim_ctrl_get_num_of_inserted()) // SIM1 and SIM2 is inserted	
    {
	VcpCommandPopup *m_CLogMakeCall;
	VFX_OBJ_CREATE(m_CLogMakeCall, VcpCommandPopup, this);
	if(srv_sim_ctrl_is_inserted(MMI_SIM1))
	{
		m_CLogMakeCall->addItem(MAKE_CLG_CALL_SIM1, VFX_WSTR_RES(STR_GLOBAL_SIM_1),VCP_POPUP_BUTTON_TYPE_NORMAL);
	}

	if(srv_sim_ctrl_is_inserted(MMI_SIM2))
	{
		m_CLogMakeCall->addItem(MAKE_CLG_CALL_SIM2, VFX_WSTR_RES(STR_GLOBAL_SIM_2),VCP_POPUP_BUTTON_TYPE_NORMAL);
	}

        if (log.data.call_log.name[0] != 0)
            m_CLogMakeCall->setText(VFX_WSTR_MEM((VfxWChar *)log.data.call_log.name));
       else
           m_CLogMakeCall->setText(VFX_WSTR_MEM((VfxWChar *)log.data.call_log.cid));	

	m_CLogMakeCall->addItem(MAKE_CLG_CANNEL, VFX_WSTR_RES(STR_GLOBAL_CANCEL), VCP_POPUP_BUTTON_TYPE_CANCEL);	
	m_CLogMakeCall->setAutoDestory(VFX_TRUE);	
	m_CLogMakeCall->show(VFX_TRUE);	
	m_CLogMakeCall->m_signalButtonClicked.connect(this, &VappCLogList::onPopupButtonClicked);
	m_CLogMakeCall->m_signalPopupState.connect(this, &VappCLogList::onPopupStateChanged);
    }
    else if(srv_mode_switch_get_network_mode(MMI_SIM1) == SRV_MODE_SWITCH_ON&&
	       srv_mode_switch_get_network_mode(MMI_SIM2) == SRV_MODE_SWITCH_OFF&&
	       1 == srv_sim_ctrl_get_num_of_inserted())
    {
	        m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE);
    }
    else if(srv_mode_switch_get_network_mode(MMI_SIM1) == SRV_MODE_SWITCH_OFF&&
	       srv_mode_switch_get_network_mode(MMI_SIM2) == SRV_MODE_SWITCH_ON&&
	       1 == srv_sim_ctrl_get_num_of_inserted())
   {
...
if(GetClgListMode() == EDIT_MODE_CLG)
{
    ...
    ret = m_clogDataSrv->getClogByID(m_currClogItemID, log);
    m_action->setLog(log);
    m_action->onDeleteItem(VppCLogAction::DELETE_ITEM);
    ...
    {
        m_action->setLog(log);
        if (m_currSIMID == MMI_SIM1)
            m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE);
        else if (m_currSIMID == MMI_SIM2)
            m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE_SIM2);
        else if (m_currSIMID == MMI_SIM3)
            m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE_SIM3);
        else if (m_currSIMID == MMI_SIM4)
            m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE_SIM4);
    }
    else
    {
        ...
        if (log.data.call_log.sim_id == MMI_SIM1)
            m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE);
        else if (log.data.call_log.sim_id == MMI_SIM2)
            m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE_SIM2);
        else if (log.data.call_log.sim_id == MMI_SIM3)
...
#if defined(__CUSTOM_IPHONE_STYLE_CALLLOG__)
void VappCLogList::IphoneUIInit()
{
	m_mode = NORMAL_MODE_CLG;
	IphoneUIInitTitle();
	IphoneUIInitBtn();
	IphoneUIInitSegBtn();
	IphoneUIClogListInit();

}


void VappCLogList::IphoneUIInitTitle()
{
    VFX_OBJ_CREATE(m_clgTitle,VcpCommandTitle,this);
    m_clgTitle->setSize(LCD_WIDTH, TITLE_HEIGHT);
    m_clgTitle->setPos(0,0);	
    m_clgTitle->setImgContent(VfxImageSrc(IMG_ID_CALLLOG_TITLE_BAR));

}

void VappCLogList::IphoneUIInitBtn()
{
    if(m_clgTitle != NULL)
    {
        VFX_OBJ_CREATE(m_DeleteBtn,VcpButton,m_clgTitle);	
	m_DeleteBtn->setAnchor(0.0f, 0.0f);
	m_DeleteBtn->setPos(TITLE_BTN_X, TITLE_BTN_Y);
	m_DeleteBtn->setPlacement(VCP_BUTTON_PLACEMENT_OVERLAP);
	m_DeleteBtn->setContentPlacement(VFX_FRAME_CONTENT_PLACEMENT_TYPE_RESIZE);
	m_DeleteBtn->setSize(TITLE_BTN_WIDHT,TITLE_BTN_HEIGH);	
	m_DeleteBtn->setBgImageList(VcpStateImage(IMG_ID_CALLLOG_EDIT_N,
											IMG_ID_CALLLOG_EDIT_P,
											IMG_ID_CALLLOG_EDIT_P,
											IMG_ID_CALLLOG_EDIT_P));
	m_DeleteBtn->setText(VFX_WSTR_RES(STR_GLOBAL_CLEAR));
	m_DeleteBtn->setTextColor(VFX_COLOR_WHITE);	
	m_DeleteBtn->setId(BOTTOM_BTN_CLEAR);
	m_DeleteBtn->setHidden(VFX_TRUE);
	m_DeleteBtn->setTextFont(VfxFontDesc(VFX_FONT_DESC_VF_SIZE(12), 
							VFX_FONT_DESC_ATTR_NORMAL, 
							VFX_FE1_1));	
	m_DeleteBtn->m_signalClicked.connect(this, &VappCLogList::onToolBarClick);
	
        VFX_OBJ_CREATE(m_EditBtn,VcpButton,m_clgTitle);	
	m_EditBtn->setAnchor(1.0f, 0.0f);
	m_EditBtn->setPos(LCD_WIDTH-TITLE_BTN_X, TITLE_BTN_Y);
	m_EditBtn->setPlacement(VCP_BUTTON_PLACEMENT_TEXT_ONLY);	
	m_EditBtn->setContentPlacement(VFX_FRAME_CONTENT_PLACEMENT_TYPE_RESIZE);
	m_EditBtn->setSize(TITLE_BTN_WIDHT,TITLE_BTN_HEIGH);	
	m_EditBtn->setBgImageList(VcpStateImage(IMG_ID_CALLLOG_EDIT_N,
											IMG_ID_CALLLOG_EDIT_P,
											IMG_ID_CALLLOG_EDIT_P,
											IMG_ID_CALLLOG_EDIT_P));
	m_EditBtn->setText(VFX_WSTR_RES(STR_GLOBAL_EDIT));
	m_EditBtn->setTextColor(VFX_COLOR_WHITE);
	m_EditBtn->setId(BOTTOM_BTN_EDIT);
	m_EditBtn->setTextFont(VfxFontDesc(VFX_FONT_DESC_VF_SIZE(12), 
							VFX_FONT_DESC_ATTR_NORMAL, 
							VFX_FE1_1));	
	m_EditBtn->m_signalClicked.connect(this, &VappCLogList::IphoneUITitleBtnClick);	

	/*finish button*/
	
        VFX_OBJ_CREATE(m_FinishBtn,VcpButton,m_clgTitle);	
	m_FinishBtn->setAnchor(1.0f, 0.0f);
	m_FinishBtn->setPos(LCD_WIDTH-TITLE_BTN_X, TITLE_BTN_Y);
	m_FinishBtn->setPlacement(VCP_BUTTON_PLACEMENT_TEXT_ONLY);	
	m_FinishBtn->setContentPlacement(VFX_FRAME_CONTENT_PLACEMENT_TYPE_RESIZE);
	m_FinishBtn->setSize(TITLE_BTN_WIDHT,TITLE_BTN_HEIGH);	
	m_FinishBtn->setBgImageList(VcpStateImage(IMG_ID_CALLLOG_FINISH_N,
												IMG_ID_CALLLOG_FINISH_P,
												IMG_ID_CALLLOG_FINISH_P,
												IMG_ID_CALLLOG_FINISH_P));	
	m_FinishBtn->setText(VFX_WSTR_RES(STR_GLOBAL_DONE));
	m_FinishBtn->setTextColor(VFX_COLOR_WHITE);
	m_FinishBtn->setId(BOTTOM_BTN_FINISHT);	
	m_FinishBtn->setTextFont(VfxFontDesc(VFX_FONT_DESC_VF_SIZE(12), 
							VFX_FONT_DESC_ATTR_NORMAL, 
							VFX_FE1_1));
	m_FinishBtn->setHidden(VFX_TRUE);
	m_FinishBtn->m_signalClicked.connect(this, &VappCLogList::IphoneUITitleBtnClick);		
    }
}

void VappCLogList::IphoneUIInitSegBtn()
{
	if(m_clgTitle !=NULL)
	{
	    VFX_OBJ_CREATE(m_segBtn, VcpSegmentButton, m_clgTitle);
	    m_segBtn->setAnchor(0.5f, 0.0f);
	    m_segBtn->setPos(TITLE_SEGBTN_X, TITLE_SEGBTN_Y);	
	    m_segBtn->setSize(TITLE_SEGBTN_W , TITLE_SEGBTN_H);
	    m_segBtn->setType(VCP_SEGMENT_BUTTON_TYPE_RADIO);
	    m_segBtn->setStyle(VCP_SEGMENT_BUTTON_STYLE_IMAGE_AND_TEXT);
	    m_segBtn->setFontSize(12);
	    // Create 2 buttoned segment button
	    VcpStateImage allCallsImg, missedCallsImg;
		allCallsImg.setImage(IMG_ID_ALL_CALLLOG_BTN_N, 
							IMG_ID_ALL_CALLLOG_BTN_P,
							IMG_ID_ALL_CALLLOG_BTN_P,
							IMG_ID_ALL_CALLLOG_BTN_P);
		missedCallsImg.setImage(IMG_ID_MISS_CALLLOG_BTN_N, 
			                                        IMG_ID_MISS_CALLLOG_BTN_P, 
			                                        IMG_ID_MISS_CALLLOG_BTN_P, 
			                                        IMG_ID_MISS_CALLLOG_BTN_P);
	    m_segBtn->addButton(SEGMENT_BTN_ALL_CALL, allCallsImg, VFX_WSTR_RES(STR_ID_IPHONEUI_CLOG_ALL));
	    m_segBtn->addButton(SEGMENT_BTN_MISSED_CALL, missedCallsImg, VFX_WSTR_RES(STR_ID_IPHONEUI_CLOG_MISSED));
	    // Connect segment button with your own callback
	    m_segBtn->m_signalButtonClicked.connect(this, &VappCLogList::onToolBarClick); // common button click func
	}
	updataToolBarStatus();
...
void VappCLogList::IphoneUIClogListInit()
{
	VfxU32 count=0;

	/*List */
	m_CLogList->setSize(getSize().width, getSize().height-TITLE_HEIGHT);
	m_CLogList->setPos(0,TITLE_HEIGHT);
	
	if(m_clogDataSrv != NULL)
	{
    		count = m_clogDataSrv->getClogCount(VAPP_CLOG_LIST_ID_ALL_CALL);
	}
	if(count > 0)
	{
   		VFX_ALLOC_MEM(m_select_state, sizeof(VfxBool)*count, this);
    		kal_mem_set(m_select_state, 0, sizeof(VfxBool)*count);
	}
	else
	{
   		VFX_ALLOC_MEM(m_select_state, sizeof(VfxBool)*1, this);
    		kal_mem_set(m_select_state, 0, sizeof(VfxBool)*1);
	}
}

void VappCLogList::updataClgTitleBarBtnStatus()
{
	if(m_mode == NORMAL_MODE_CLG)
	{
		if(m_EditBtn != NULL)
		{
	...
	else
	{
		...
		if(m_DeleteBtn != NULL)
		{
			if(m_clogDataSrv->getClogCount(m_currListID) >0)
			{
				m_DeleteBtn->setHidden(VFX_FALSE);
			}
			else
			{
				m_DeleteBtn->setHidden(VFX_TRUE);
		...
		if(m_FinishBtn != NULL)
		{
			m_FinishBtn->setHidden(VFX_FALSE);
		}
	}
}

void VappCLogList::SetClgListMode(VappClogListModeEnum mode)
{
	m_mode = mode;
}

VappClogListModeEnum VappCLogList::GetClgListMode()
{
	return m_mode;
}

void VappCLogList::IphoneUITitleBtnClick(VfxObject* sender, VfxId id)
{
	VcpButton* button = VFX_OBJ_DYNAMIC_CAST(sender, VcpButton);
	switch(id)
	{
		case BOTTOM_BTN_EDIT:
			m_mode = EDIT_MODE_CLG;	
			break;
		case BOTTOM_BTN_FINISHT:
			m_mode = NORMAL_MODE_CLG;
			break;
		default:
			/*do nothing*/
	...
	SetClogListStyle(m_mode);
	updataClgTitleBarBtnStatus();	
}

void VappCLogList::onPopupStateChanged(VfxBasePopup* popup, VfxBasePopupStateEnum state)
{
	if (state == VFX_BASE_POPUP_AFTER_END_ANIMATION)
	{
		switch(m_CurrentCmdBtnId)
		{
			case MAKE_CLG_CALL_SIM1:
       			 	m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE);				
				break;
			case MAKE_CLG_CALL_SIM2:
        				m_action->makeCall(SRV_UCM_VOICE_CALL_TYPE_SIM2);				
				break;
			default:
				break;
...
void VappCLogList::onPopupButtonClicked(VfxObject* obj, VfxId id)
{
	m_CurrentCmdBtnId = id;
}

void VappCLogList::SetClogListStyle(VappClogListModeEnum style)
{
	VfxU32 count=0;

	if(style ==NORMAL_MODE_CLG)
	{
		m_CLogList->setMenuMode(VCP_LIST_MENU_MODE_NORMAL, VFX_TRUE);	
        		m_CLogList->setMenuControlMode(VCP_LIST_MENU_CONTROL_MODE_CMD_BUTTON, VFX_TRUE);	
	}
	else
	{
		//m_CLogList->setMenuMode(VCP_LIST_MENU_MODE_HEAD_SINGLE_CHECK_MARK, VFX_FALSE);
        		//m_CLogList->setMenuControlMode(VCP_LIST_MENU_CONTROL_MODE_NORMAL, VFX_FALSE);			
		m_CLogList->setMenuMode(VCP_LIST_MENU_MODE_HEAD_SINGLE_DELETE_MARK, VFX_FALSE);		
        		m_CLogList->setMenuControlMode(VCP_LIST_MENU_CONTROL_MODE_CMD_BUTTON_EX, VFX_TRUE);			
	}
...
VcpListMenuModeEnum VappCLogList::GetClogListStyle()
{
	return m_CLogList->getMenuMode();
}

VcpListMenuItemStateEnum VappCLogList::getItemState(VfxU32 index) const
{
...
void VappCLogList::onSelectionChanged(VcpListMenu *menu, VfxU32 index, VcpListMenuItemStateEnum newState)
{
...
void VappCLogListMenuItemCell::onLayoutElements()
{
	VfxWString string3;
	VfxSize stringSize3;	   
	if (getText(VCP_LIST_MENU_FIELD_SUB_TEXT1, string3))
	{
		stringSize3 = VfxFontDesc(VCP_LIST_MENU_FONT_SIZE_SMALL).measureStr(string3);
	}
	if(m_textFrame3)
	{
		if (m_imageFrame)
		{
			m_imageFrame->setRect(VfxRect(CT_IMG_LEFT_GAPS + CT_IMG_RIGHT_GAPS +stringSize3.width, 
				getRect().getHeight() - 1 - TEXT3_BOTTOM_GAPS - m_textFrame3->getRect().getHeight()+2,
	...
	maxText1Width = getRect().getWidth() - (CT_IMG_LEFT_GAPS + CT_IMG_RIGHT_GAPS + CT_IMG_MAX_WIDTH) - TEXT1_LEFT_GAPS
		- TEXT1_RIGHT_GAPS - textFrame4AreaWidth - textFrame2AreaWidth-CT_TEXT_LEFT_GAPS-15;
	...
	if (m_textFrame1)
	{
		m_textFrame1->setRect(VfxRect(CT_TEXT_LEFT_GAPS, 
			TEXT1_TOP_GAPS,
	...
	if (m_textFrame3)
	{
		m_textFrame3->setRect(VfxRect(CT_TEXT_LEFT_GAPS, 
	...
	if (m_textFrame5)
	{
		...
		m_textFrame5->setAnchor(0.0f, 0.5f);
		m_textFrame5->setPos(getRect().getWidth() - 1 - TEXT5_RIGHT_GAPS - m_textFrame5->getRect().getWidth(), 
			getRect().getHeight()/2);		
...
void VappCLogListMenuItemCell::onUpdateTextFrameStyle(VcpListMenuFieldEnum fieldType, VfxTextFrame *frame)
{
	VcpListMenuCellIconQuintTextFrame::onUpdateTextFrameStyle(fieldType, frame);
	if (fieldType == VCP_LIST_MENU_FIELD_TEXT)
	{
		frame->setFont(VfxFontDesc(VFX_FONT_DESC_VF_SIZE(19),VFX_FONT_DESC_ATTR_NORMAL,VFX_FE1_3));
		frame->setColor(VFX_COLOR_RED);
	...
	{ 
		frame->setFont(VfxFontDesc(VCP_LIST_MENU_FONT_SIZE_SMALL,VFX_FONT_DESC_ATTR_NORMAL,VFX_FE1_1));/*tangjinxing add*/	
		frame->setAlignMode(VfxTextFrame::ALIGN_MODE_RIGHT);
	}
}

void VappCLogListMenuItemCell::onUpdateTextViewStyle(VcpListMenuFieldEnum fieldType, VcpTextView *frame)
{
	VcpListMenuCellIconQuintTextFrame::onUpdateTextViewStyle(fieldType, frame);
	if (fieldType == VCP_LIST_MENU_FIELD_TEXT)
	{
		frame->setFont(VfxFontDesc(VFX_FONT_DESC_VF_SIZE(19),VFX_FONT_DESC_ATTR_NORMAL,VFX_FE1_3));		
...
#else/*__CUSTOM_IPHONE_STYLE_CALLLOG__*/
void VappCLogListMenuItemCell::onLayoutElements()
{
    if (m_imageFrame)
    {
...
/*****************************************************************************
 * Class VappCLogListEx
 *****************************************************************************/

/*
VFX_IMPLEMENT_CLASS("VappCLogListEx", VappCLogListEx, VfxPage);

VappCLogListEx::VappCLogListEx() :
    m_CLogList(NULL),
    m_currPageType(VAPP_CLOG_PAGE_FOR_DIALER),
    m_currClogItemID(0),
    m_action(NULL),
    m_storeIndex(0),
    m_clogDataSrv(NULL),
...
VappCLogListEx::VappCLogListEx(VappCLogPageTypeEnum pageType,           
                 VfxU16 store_idx,          // only for CLOG_PAGE_FOR_CONTACT
                 mmi_sim_enum sim_id // only for CLOG_PAGE_FOR_DIALER
                 ) :
    m_CLogList(NULL),
    m_currPageType(pageType),
    m_currClogItemID(0),
    ...
    m_currSIMID(sim_id)
{
    MMI_TRACE(VAPP_CLOG_TRC_INFO, TRC_VCLOG_LIST_CONSTRUCTOR, pageType, 20, store_idx, sim_id);
}


void VappCLogListEx::onInit()
{
    VfxPage::onInit(); // call base class
    ...
    {        
        ...
        m_CLogList->m_signalItemTapped.connect(this, &VappCLogListEx::onItemShortTapped);
        
        m_clogDataSrv->m_signalClogUpdated.connect(this, &VappCLogListEx::onUpdateClogList);
    }
    else //VAPP_CLOG_PAGE_FOR_CONTACT
    {
        VFX_OBJ_CREATE_EX(m_clogDataSrv, VappCLogDataProvider, this, (VAPP_CLOG_PAGE_FOR_CONTACT));
        ...
        m_clogDataSrv->m_signalGetClogCbDone.connect(this, &VappCLogListEx::onUpdatePageForContact);
        m_clogDataSrv->m_signalClogUpdated.connect(this, &VappCLogListEx::onUpdateClogList);
        mmi_frm_cb_reg_event(EVT_ID_PHB_UPD_CONTACT, &VappCLogListEx::onPhbContactChanged, this);
    }
    regListener();
}


void VappCLogListEx::onDeinit()
{
    deregListener();
    if (m_currPageType == VAPP_CLOG_PAGE_FOR_DIALER)
    {
        m_clogDataSrv->m_signalClogUpdated.disconnect(this, &VappCLogListEx::onUpdateClogList);
    }
    else // VAPP_CLOG_PAGE_FOR_CONTACT
    {
        m_clogDataSrv->m_signalGetClogCbDone.disconnect(this, &VappCLogListEx::onUpdatePageForContact);
        m_clogDataSrv->m_signalClogUpdated.disconnect(this, &VappCLogListEx::onUpdateClogList);
        mmi_frm_cb_dereg_event(EVT_ID_PHB_UPD_CONTACT, &VappCLogListEx::onPhbContactChanged, this);
    }
...
void VappCLogListEx::onEnter(VfxBool isBackward)
{
    MMI_TRACE(VAPP_CLOG_TRC_INFO, TRC_VCLOG_LIST_ONENTER, m_currPageType, isBackward);
    
    m_showTimer->setStartDelay(1);
    m_showTimer->m_signalTick.connect(this, &VappCLogListEx::showlistscreen);
    m_showTimer->start();
...
void VappCLogListEx::showlistscreen(VfxTimer *timer)
{
    m_clogDataSrv->clearUnreadMissedCount();
}


void VappCLogListEx::onExit(VfxBool isBackward)
{
    m_showTimer->stop();
}


VfxScrRotateTypeEnum VappCLogListEx::onQueryRotate(const VfxScreenRotateParam &param)
{
    if (VAPP_CLOG_PAGE_FOR_CONTACT == m_currPageType)
        return param.rotateTo;
    else
        return VFX_SCR_ROTATE_TYPE_NORMAL;
}


mmi_ret VappCLogListEx::onPhbContactChanged(mmi_event_struct *param)
{
    VappCLogListEx *currObj = (VappCLogListEx*)param->user_data;
...
void VappCLogListEx::regListener()
{
    // Connect signal to refresh the time string.
    VdatTimeNotification *systemTimeSrv = VFX_OBJ_GET_INSTANCE(VdatTimeNotification);
    systemTimeSrv->m_signalTimeChanged.connect(this, &VappCLogListEx::onTimeChanged);
}


void VappCLogListEx::deregListener()
{
    // DisConnect signal to refresh the time string.
    VdatTimeNotification *systemTimeSrv = VFX_OBJ_GET_INSTANCE(VdatTimeNotification);
    systemTimeSrv->m_signalTimeChanged.disconnect(this, &VappCLogListEx::onTimeChanged);
}


mmi_ret VappCLogListEx::onProc(mmi_event_struct *evt)
{
    if (m_action != NULL)
        return m_action->onProc(evt);
...
void VappCLogListEx::onTimeChanged(VfxU32 flag)
{
    if (flag & VDAT_SYSTIME_CHANGED_DAY)
    {
        onUpdateClogList(NULL);
...
void VappCLogListEx::onUpdateClogList(void *evt)
{
    MMI_TRACE(VAPP_CLOG_TRC_INFO, TRC_VCLOG_LIST_ONUPDATECLOGLIST, 
        m_currPageType,
        20,
...
void VappCLogListEx::onUpdatePageForContact()
{
    MMI_TRACE(VAPP_CLOG_TRC_INFO, TRC_VCLOG_LIST_ONUPDATEPAGEFORCONTACT, 
        m_currPageType,
        m_clogDataSrv->getClogNum());
...
VfxBool VappCLogListEx::getItemText(
        VfxU32 index,                    // [IN] the index of item
        VcpListMenuFieldEnum fieldType,  // [IN] the type of the field in the cell
        VfxWString &text,                // [OUT] the text resource
        VcpListMenuTextColorEnum &color  // [OUT] the text color
...
VfxBool VappCLogListEx::getItemImage(
        VfxU32 index,                    // [IN] the index of item
        VcpListMenuFieldEnum fieldType,  // [IN] the type of the field in the cell
        VfxImageSrc &image               // [OUT] the image resource
        )
...
VfxU32 VappCLogListEx::getCount() const
{
    VfxU32 count;
    count = m_clogDataSrv->getClogCount(VAPP_CLOG_LIST_ID_ALL_CALL);
    return count;
...
VcpListMenuCellClientBaseFrame * VappCLogListEx::getItemCustomContentFrame(
        VfxU32 index,        // [IN] index of the item
        VfxFrame *parentFrame // [IN] parent frame of the custom content frame
        ) 
{
...
VfxBool VappCLogListEx::getMenuEmptyText(
                            VfxWString &text,                // [OUT] the text resource
                            VcpListMenuTextColorEnum &color  // [OUT] the text color
                            )
{
...
VfxBool VappCLogListEx::isToday(const VfxU32 &timestamp, VfxDateTime &logTime) const
{
    applib_time_struct startTime; 
    VfxU32 local_sec = applib_dt_sec_utc_to_local(timestamp);
	mmi_dt_utc_sec_2_mytime(local_sec, &startTime, MMI_FALSE);
...
void VappCLogListEx::onItemShortTapped(
        VcpListMenu *list,          // [IN] ListMenu pointer
        VfxU32 index                // [IN] The selected menu item
    )
{
