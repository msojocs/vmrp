...
#include "cp\vcp_form.h"
#include "cp/vcp_tabctrl_page.h"
#include "cp/vcp_navi_title_bar.h"
//#include "mmi_rp_vtst_sample_def.h"

#include "vfx_mc_include.h"
#include "vfx_uc_include.h"
#include "vcp_include.h"
#include "cp\vcp_popup.h"
#include "cp\vcp_form.h"
#include "cp\vcp_button_util.h"
...
extern "C" mmi_id vcui_unifiedcomposer_create(mmi_id parent_id, VappUcEntryStruct *data)
{
    ...
    cui_id = VfxAppLauncher::createCui(
        ...
        parent_id,
        (void*)g_app.initData, (g_app.initData)?sizeof(VappUcEntryStruct):0);
...
/* under construction !*/
	...
	/*	else if((operation_type == UC_OPERATION_SEND_VBOOKMARK_AS_SMS))
		{
...
/* SMS will handle the following part */
/*
    if(result == MMI_TRUE)
    {
        if (send_info.need_highlight_ind_to_um == VFX_TRUE)
        {
            mmi_um_highlight_msg_ind(SRV_UM_MSG_SMS, mmi_msg_get_um_type_by_msgid(*msg_id), *msg_id);
            send_info.need_highlight_ind_to_um = VFX_FALSE;
        }
        srv_uc_send_um_msg_refresh_ind(main.instance, mmi_msg_get_um_type_by_msgid(*msg_id), SRV_UM_MSG_SMS);
    }
*/
...
extern "C" void ucDeleteSmsCallback(srv_sms_callback_struct* callback_data)
{
    ...
    {
		if((app->main.instance >= 0 && app->main.instance < MAX_UC_INSTANCES) && 
		   (app->main.msg_operation >= 0 && app->main.msg_operation < UC_OPERATION_TOTAL_NUM) &&
		   (app->main.highlighted_object_type >= 0 && app->main.highlighted_object_type < SRV_UC_DONE_TYPE_TOTAL_NUM))
		/*make temp condition*/
		{
        ...
        object && object->type != SRV_UC_OBJECT_TYPE_ATTACHMENT)
    {
             ...
             &fail_in_restricted_size_check))
        {
            if (new_slide_size > 0)
            {
                ...
                m_service_cntxt.msg->current_slide_num = slide->slide_num;
            }
				...
				else
				{
					...
					ASSERT(cur_slide->txt.object->file_path_ucs);
			...
			#endif /*__MMI_EMAIL__*/
						{
							...
							{
								m_service_cntxt.main->text_buffer[UC_MMS_MAX_TEXT_CHAR_COUNT*2] = '\0' ;
								m_service_cntxt.main->text_buffer[(UC_MMS_MAX_TEXT_CHAR_COUNT*2)+1] = '\0' ;
								m_service_cntxt.main->text_buffer[(UC_MMS_MAX_TEXT_CHAR_COUNT*2)+2] = '\0' ;
        ...
        case SRV_UC_STATE_EDIT_EXISTED_MSG:
         {
            ...
            m_service_cntxt.msg->current_slide_num = m_service_cntxt.msg->msg_body.curr_slide->slide_num;
...
#endif
    msg_type = determineMsgTypeByContent();
...
VfxBool VcuiUcMainCui::handleData(void* args, VfxU32 argsize)
{
    ...
    //else if(data->file_path) /* WRITE MESSAGE from filemanager */
    {
        ...
        else if(data->operation == UC_OPERATION_WRITE_NEW_MMS_TEMPLATE)
        {
        ...
        else if(data->info_type == SRV_UC_INFO_TYPE_EMAIL && data->type == SRV_UC_STATE_EDIT_EXISTED_MSG )
        {
        #ifdef __MMI_EMAIL__
...
extern "C" 
{
	extern kal_int32 mr_sms_get_reply_flag(void);
	extern  void mr_sms_set_reply_flag(kal_int32 value);
	kal_int32 mr_sms_get_simid(void);
...
*/
    ...
    if (data->type != SRV_UC_STATE_WRITE_NEW_MSG)
    {
        ...
        #ifdef __MMI_DSM_NEW__
		if(mr_sms_get_reply_flag())
		{
#if (__MR_CFG_VAR_MTK_VERSION__ >= 0x11B1224)		
	             m_service_cntxt->send_info->send_sim_id= (srv_uc_sim_id)mr_sms_get_simid();
#else
	             m_service_cntxt.send_info->send_sim_id= (srv_uc_sim_id)mr_sms_get_simid();			
#endif
	             mr_sms_set_reply_flag(0);
		}
        #endif
...
void VappUcAppData::ucEntryPreviousSlide(void)
{
    ...
    /*----------------------------------------------------------------*/
    mma_mms_slide_struct *slide = m_service_cntxt.msg->msg_body.curr_slide;
    mma_mms_slide_struct* pre_slide = NULL;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
   // ASSERT(slide->prev);
...
void VappUcAppData::vappUcSdPlugOutHdlr(void)
{
    ...
    BOOL local_flag_hide_sig_confirm = MMI_FALSE;
    ...
    if (m_service_cntxt.send_info->new_msg_id)
    {
        if (send_info.new_msg_storage != MMA_MSG_STORAGE_PHONE)
        {
            ...
            if(m_service_cntxt.send_info->action == SRV_UC_ACTION_PREVIEW)
            {
                ...
                if((m_service_cntxt.send_info->existed_msg_type == SRV_UC_MSG_TYPE_MMS_PREFER &&
                    m_service_cntxt.send_info->existed_msg_id && 
    ...
    if (close_uc)
    {
		...
		{
        VcuiUcMainCui* app = (VcuiUcMainCui *)m_parent;
        /*if(m_service_cntxt.send_info->fs)
        {
            FS_Close(m_service_cntxt.send_info->fs);
            m_service_cntxt.send_info->fs =0;
        }*/
        app->vappUcTerminate();
        //resetApp();
        /* TODO: See this
...
//        srv_uc_reset_layout_if_needed(main.instance);
    ...
    if (m_service_cntxt.main->file_path && FS_GetAttributes((WCHAR*) m_service_cntxt.main->file_path) < 0)
    {
        /* TODO: See about these
        mmi_uc_delete_screen(SCR_ID_UC_CONFIRM);
    ...
    if (attach_removed && !m_service_cntxt.msg->msg_body.attachment)
    {
        /* TODO: See about these
...
void VappUcAppData::ucSetInsertMenu(void)
{
    ...
    mmi_frm_unhide_menu_item(MENU_ID_VUC_OPT_INSERT_PHB_NAME);
    mmi_frm_unhide_menu_item(MENU_ID_VUC_OPT_INSERT_PHB_NUMBER);
	/*Cosmos Opera Turbo */
  ...
  //          /* EMAIL disallow to add other media */
   //        mmi_frm_hide_menu_item(MENU_ID_VUC_OPT_INSERT_PHB_NAME);
   //        mmi_frm_hide_menu_item(MENU_ID_VUC_OPT_INSERT_PHB_NUMBER);
   //        mmi_frm_hide_menu_item(MENU_ID_VUC_OPT_INSERT_BOOKMARK);
           mmi_frm_hide_menu_item(MENU_ID_VUC_OPT_SLIDE);
           mmi_frm_hide_menu_item(MENU_ID_VUC_OPT_SLIDE_INSERT);
		...
		case EVT_ID_SRV_FMGR_NOTIFICATION_DEV_PLUG_OUT:
			...
			if(drive_counter)
			{
                ...
                while(instance < MAX_UC_INSTANCES)
                {
                    if(g_app.app_data[instance])
                    {
        				g_app.app_data[instance]->vappUcSdPlugOutHdlr();
                    }
                    instance++;
                }
			}
...
extern "C" VfxBool vappUcFlightModecheck(void)
{
   ...
   return g_app.is_flight_mode_enable ;
}
/*kuldeep_gplus*/
...
extern "C" WCHAR* vappUcGetResizeResultPopupStr(S32 result, mmi_nmgr_balloon_type_enum *type)
{
    ...
    //Just to avoid warning. Extra line.
    return (WCHAR*)((UI_string_type) GetString(STR_GLOBAL_NOT_ENOUGH_MEMORY));
}
...
extern "C" srv_uc_addr_struct *vapp_uc_get_sms_address_callback(U16 index, void* user_data)
{
    ...
    VappUcEntryStruct *uc_data = app->otherAppTempUcInfo;
...
VfxBool vapp_uc_is_ascii_string(const S8 *ucs2_str)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    utf8_len = mmi_chset_ucs2_to_utf8_string(utf8_str, utf8_buffer_size, (U8*)ucs2_str);
	number = utf8_str;
