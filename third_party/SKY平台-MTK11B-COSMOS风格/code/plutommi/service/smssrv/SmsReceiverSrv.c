...
#if defined(PBK_IBK_SUPPORT)
	{
	extern void pbk_set_sms_context(char* number, U8* data, S32 len, U32 seg, U32 total_seg);
	pbk_set_sms_context((char*)app_lib_data->forMMI_TPAddr.number, app_lib_data->forMMI_UserData, (S32)app_lib_data->forMMI_UserData_length, (U32)app_lib_data->tpdu.concat_info.seg, (U32)app_lib_data->tpdu.concat_info.total_seg);
	}
#endif
...
static void srv_sms_save_sms_callback(srv_sms_callback_struct* data)
{
    ...
    if (data->result == MMI_FALSE)
    {
        ...
        if (data->cause == SRV_SMS_CAUSE_MEM_FULL)    
        {
            ...
            if (srv_sms_is_memory_full(SRV_SMS_STORAGE_UNSPECIFIC, general_data->sim_id))
            {
	            ...
	            if(!srv_sms_get_unused_space(SRV_SMS_STORAGE_SIM, general_data->sim_id))
                {
    ...
    {    
        ...
        else
        {
            ...
            event_data->content = OslMalloc(app_lib_data->forMMI_UserData_length+2);
            memset(event_data->content,0,app_lib_data->forMMI_UserData_length+2);
            memcpy(event_data->content,app_lib_data->forMMI_UserData,app_lib_data->forMMI_UserData_length);
            srv_sms_emit_event(EVT_ID_SRV_SMS_NEW_MSG, event_data);
            OslMfree(event_data->content);
#else
            srv_sms_emit_event(EVT_ID_SRV_SMS_NEW_MSG, event_data);
#endif
            OslMfree(event_data);
...
static void srv_sms_timeout_concatenate_data(void)
{
    ...
    {
        StartTimer(MESSAGES_LMS_RECV_TIMER_ID, SRV_SMS_LMS_TIME_OUT, srv_sms_timeout_concatenate_data);
    }
    else
    {
