...
static void srv_backlight_lcd_sleep_out(void)
{
    ...
    if (is_in_sleep_mode)
    {
        ...
        kal_sleep_task(4); // sleep 17ms, to wait blt finish, then GPIO can turn on backlight, help LCD blt workaround
    }
#endif /* !defined(__MMI_GPIO_DISABLE_SLEEP_MODE__) */        
}
...
void srv_backlight_turn_on(srv_backlight_timer_type_enum time_enum)
{
    ...
    /* [MMIGPIO]srv_backlight_turn_on(),SYNC_MODE=%d,timer_enum=%d,timer_status=%d,lcd_state=%d,sublcd_state=%d */
    MMI_TRACE(MMI_COMMON_TRC_G8_DEV, TRC_MMI_GPIO_TURNON_BACKLIGHT,
                opt_flag, time_enum, g_gpio_p->timer_status, g_gpio_p->lcd_state, g_gpio_p->sublcd_state);
...
/* Please refer to GpioSrvGprot.h                                           */
/*--------------------------------------------------------------------------*/
MMI_BOOL srv_backlight_is_ctrl(void)
{
    return g_srv_backlight_is_ctrl;
}
...
void srv_backlight_ctrl_req_hdlr(void *msg, int mod_id , void *peer_buf)
{
    ...
    /*----------------------------------------------------------------*/
    StopTimer(BACKLIGHT_TIMER);
    g_srv_backlight_is_ctrl = MMI_TRUE; // grap backlihgt control
    ...
    if (req->on_off)
    {
        ...
        {
            g_srv_backlight_is_ctrl = MMI_FALSE; // Release backlihgt control
