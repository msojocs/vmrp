...
#ifdef __MMI_DM_CREAT_NEW_LAYER_OUTMEM__
static U8 *new_layer_mem_buffer_ptr=NULL;
static U8  new_layer_mem_buffer_alloc_type=0;//0 from gdi,1 from screen mem,2 from med ext mem
#endif
U8 old_bg_opacity = 0;
U16 old_image_id = 0;
...
static void dm_new_layer_alloc_outmem(S32 size)
{
	if(new_layer_mem_buffer_ptr==NULL)
	{
		new_layer_mem_buffer_ptr=(U8 *)mmi_frm_scrmem_alloc_framebuffer(size);
		if(new_layer_mem_buffer_ptr!=NULL)
			new_layer_mem_buffer_alloc_type=1;
		else
		{
			new_layer_mem_buffer_ptr=(U8 *)med_alloc_ext_mem(size);
			if(new_layer_mem_buffer_ptr!=NULL)
				new_layer_mem_buffer_alloc_type=2;
			else
				ASSERT(0);
		}
	}
}
static void dm_new_layer_free_outmem(void)
{
	if(new_layer_mem_buffer_ptr)
	{
		switch(new_layer_mem_buffer_alloc_type)
		{
			case 1:
				mmi_frm_scrmem_free((void*)new_layer_mem_buffer_ptr);
			break;
			case 2:
				med_free_ext_mem((void **)&(new_layer_mem_buffer_ptr));
			break;
			default:
			break;
		}
	}
	new_layer_mem_buffer_ptr=NULL;
	new_layer_mem_buffer_alloc_type=0;
}
#endif
...
void dm_reset_context(void)
{
    ...
    if (g_dm_cntx.is_context_reset == MMI_FALSE)
    {
        ...
        if (!(g_dm_data.s32flags & DM_SUB_LCD_CONTEXT))
        {
            ...
            {
                if (dm_layers[idx] != g_dm_scr_bg_cntx.layer_handle && (dm_layers[idx] != GDI_ERROR_HANDLE && idx != dm_wallpaper_layer_idx))
                {
        ...
        if (!dm_is_fixed_layer_mode_enabled())
        {
                ...
                g_dm_cntx.enable_small_screen_flatten == DM_SMALL_SCREEN_FLATTEN_ON)
            {
                    ...
                    dm_layers[3] != GDI_ERROR_HANDLE)
                {
                    ...
                    if (!wgui_is_wallpaper_on_bottom() && old_on_idle_screen == 0 && (dm_layers[0] == g_dm_scr_bg_cntx.layer_handle || dm_wallpaper_layer_idx == 0))
                    {
...
#ifdef __MMI_DM_CREAT_NEW_LAYER_OUTMEM__
        if (dm_layer_info.Flags & DM_NEW_LAYER_OUTMEM)
        {
        		S32 buff_size=dm_layer_info.s16Width*dm_layer_info.s16Height*2;
				dm_new_layer_alloc_outmem(buff_size);
...
/* under construction !*/
    ...
    if (!(g_dm_data.s32flags & DM_NO_BLT))
    {
        ...
        if (g_dm_scr_bg_cntx.scr_bg_layer_exist == MMI_TRUE || 
            on_idle_screen == 1 || 
            (g_dm_scr_bg_cntx.scr_bg_layer_exist == MMI_FALSE && on_idle_screen == 0 && dm_layers[0] != g_dm_scr_bg_cntx.layer_handle && dm_wallpaper_layer_idx != 0))
        {
            gdi_layer_blt(
                dm_layers[0],
                dm_layers[1],
    ...
    for (idx = 0; idx < WGUI_STATUS_ICON_BAR_TOTAL_BAR; idx++)
    {
        ...
        {
            gdi_layer_push_and_set_active((gdi_handle)wgui_status_icon_bar_get_target_layer((U8)idx));
...
{
	extern void dsm_set_title(U16 * * pTitle);
	dsm_set_title((U16 * *)&MMI_title_string);
}
#endif
}
...
GDI_RESULT dm_create_layer(S32 x, S32 y, S32 width, S32 height, gdi_handle *handle_ptr, U32 flags)
{
    ...
    switch (flags)
    {
        case DM_LAYER_BOTTOM:
            ...
            {
                if ((act_dm_layers[idx - 1] == g_dm_scr_bg_cntx.layer_handle) || (act_dm_layers[idx -1 ] != GDI_ERROR_HANDLE && (idx -1) == dm_wallpaper_layer_idx))
                {
                    break;
                }
 ...
 *  void
 *****************************************************************************/
void dm_add_title_id(MMI_STR_ID title_str_id, MMI_IMG_ID title_img_id)
{
