...
static void wgui_cat033_draw_region_with_wallpaper(S32 x1, S32 y1, S32 x2, S32 y2)
{
	...
	if(mr_backstage_draw_idle_background(dm_get_wallpaper_layer(),0) != 0)
		return;
#endif
    ...
    if (network_name != NULL)
    {
       ...
       gdi_push_and_set_alpha_blending_source_layer(wallpaper_layer);
        ...
        else
        {
            ...
            x = (r2lMMIFlag) ? (x - img_width) : (x + img_width);
         //   y += 1; /* border text */
            w = UI_device_width - (w_inset * 2) - img_width - 2/* border text */;
...
static MMI_BOOL wgui_cat033_show_network_name(void)
{
    ...
    {
        S32 width = 0, x = 0, y = 0, img_width = 0, min_y = UI_device_height - 1, i = 0, img_height = 0;
        S32 w_inset = MMI_IDLE_STRING_MARGIN_GAP;
        S32 max_str_h = Get_CharDisplayHeightOfAllLangAndType(network_name_font->size);
#if defined(__MMI_DUAL_SIM_MASTER__) && !defined(__MMI_WLAN_FEATURES__) 
        U8 *img_src = NULL;
...
static void wgui_cat033_show_details(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_DSM_NEW__
#ifdef __MMI_DSM_IDLE_WEATHER__
	t_mr_weather_info *weather_info;
#endif
#endif
    gdi_layer_lock_frame_buffer();
...
#ifdef __MMI_DSM_NEW__
#ifdef __MMI_DSM_IDLE_WEATHER__
	weather_info = dsmGetWeatherInfo();
	ShowWeaterInfo(weather_info);
#endif
#endif
    gdi_layer_unlock_frame_buffer();
    gdi_lcd_repaint_all();
...
#else /* __MMI_UI_TECHNO_IDLESCREEN_BAR__ */
    #if defined(__MMI_KLG__) || defined(__MMI_UI_STATUS_BAR_AT_BOTTOM__) || (defined(__MMI_MAINLCD_320X240__) && !defined(__MMI_UI_TECHNO_IDLESCREEN_BAR__))
...
#define WGUI_CAT30_PLAY_BUTTON_BAR_SEQ (300)

#define WGUI_CAT30_LONG_PRESS_SIMULATION_INTERVAL (500)
...
static void wgui_cat30_init_date_time_bar(void)
{
    ...
    wgui_clock_set_date_text_font(wgui_cat30_clock_date, &MMI_small_font);
    wgui_clock_set_hide_function(wgui_cat30_clock_date, wgui_cat30_hide_date_time_bar);
    wgui_clock_measure_date_text(wgui_cat30_clock_date, &date_width, &date_height, &MMI_small_font);
    ...
    wgui_clock_set_date_text_font(wgui_cat30_clock_day, &MMI_small_font);
    wgui_clock_set_hide_function(wgui_cat30_clock_day, wgui_cat30_hide_date_time_bar);
    wgui_clock_measure_date_text(wgui_cat30_clock_day, &day_width, &day_height, &MMI_small_font);
...
void wgui_cat30_move_and_draw_unlock_bar(S32 x, S32 y)
{
    ...
    button_image = get_image((U16)(IMG_KEYPAD_LOCK_UNLOCK_BUTTON0 + wgui_cat30_cntx_obj.unlock_bar_image_index));
    gui_measure_image(bk_image, &bk_image_w, &bk_image_h);
...
/* under construction !*/
    ...
    gui_show_image(button_x,button_y, button_image);
    gdi_pop_and_restore_alpha_blending_source_layer();
