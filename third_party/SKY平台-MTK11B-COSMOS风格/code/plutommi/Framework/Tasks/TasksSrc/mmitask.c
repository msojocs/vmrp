            ...
            while (count > 0)
            {
                ...
                if (OslReadCircularQ(&Message))
                {
                    ...
                    else
                    {
						...
						case MSG_ID_MPCHAT_AUD_DATA_IND:
						{
							extern void mr_media_record_data_hdlr(void* msg);
							mr_media_record_data_hdlr((void *)Message.oslDataPtr);		
							break;
						}
						#endif
						#if defined(__DSM_SPLAYER__) && defined(__MTK_TARGET__)
						case MSG_ID_SPLAYER_DISPLAY:
						{
							extern void mmi_vdoply_display_mmi(kal_uint32 full_screen);
							mmi_vdoply_display_mmi(0);
							break;
						}
						#endif
						//__MMI_DSM_NEW__ end								
...
void InitHardwareEvents(void)
{
    ...
    SetProtocolEventHandler(srv_charbat_get_bat_level_req, MSG_ID_MMIAPI_GET_BATTERY_LEVEL_REQ);
    SetProtocolEventHandler(srv_charbat_start_notify_bat_level_req, MSG_ID_MMIAPI_BATTERY_LEVEL_NOTIFY_START_REQ);
    SetProtocolEventHandler(srv_charbat_stop_notify_bat_level_req, MSG_ID_MMIAPI_BATTERY_LEVEL_NOTIFY_STOP_REQ);   
