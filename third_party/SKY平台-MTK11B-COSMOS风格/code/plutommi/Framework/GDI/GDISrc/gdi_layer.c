...
__align(32) U32 gdi_base_layer_buffer_pool[(((AL8(GDI_LCD_WIDTH)*AL8(GDI_LCD_HEIGHT))>>3) + 3) / 4];
#else
/* 32-byte alignment for ARM9 cache */
__align(32) U32 gdi_base_layer_buffer_pool[(GDI_MAINLCD_BUFFER_SIZE + 3) / 4];
#endif
                ...
                U32 buf_size)
{
    ...
    GDI_TRACE(GDI_TRC_PAIRED_LAYER, GDI_TRC_LAYER_CREATE,
        id, handle, cf,offset_x,offset_y,width,height,is_double_layer,buf_size,buf_ptr,buf_ptr1);
    ...
    if ((GDI_COLOR_FORMAT_32 == layer->cf) || (GDI_COLOR_FORMAT_32_PARGB== layer->cf))
    {
...

    {
        ...
        switch (layer->cf)
        {
            ...
            case GDI_COLOR_FORMAT_32_PARGB:
                hw_cf = GDI_HW_COLOR_FORMAT_32_PARGB;
                break;
            case GDI_COLOR_FORMAT_UYVY422:
                hw_cf = GDI_HW_COLOR_FORMAT_UYVY422;
                break;
            default:
...
GDI_RESULT gdi_layer_set_color_format(gdi_color_format cf)
{
    ...
    if ((GDI_COLOR_FORMAT_32 == gdi_act_layer->cf) || (GDI_COLOR_FORMAT_32_PARGB== gdi_act_layer->cf))
    {
        layer_info->opacity_enable = KAL_TRUE;
        layer_info->opacity_value = 0xFF;
    }
...
GDI_RESULT gdi_layer_set_position(S32 offset_x, S32 offset_y)
{
    ...
    gdi_act_layer_info->x_offset = (offset_x + GDI_LAYER_OFFSET);
    gdi_act_layer_info->y_offset = (offset_y + GDI_LAYER_OFFSET);
...
GDI_RESULT gdi_layer_set_blt_rect(U16 x, U16 y, U16 width, U16 height)
{
    ...
    LIMIT_BOUND(x, 0, x_max_limit);
    LIMIT_BOUND(y, 0, y_max_limit);
...
GDI_RESULT gdi_layer_set_opacity(BOOL opacity_enable, U8 opacity_value)
{
    ...
    GDI_ENTER_CRITICAL_SECTION(gdi_layer_set_opacity)
        ...
        {
            gdi_act_layer_info->opacity_enable = KAL_TRUE;
    ...
    gdi_act_layer_info->opacity_enable = (kal_bool)opacity_enable;
#ifdef LCD_MAX_OPACITY
    gdi_act_layer_info->opacity_value = opacity_value * LCD_MAX_OPACITY / 255;  /* shift 3, hw only support 5 bit */
#else 
    gdi_act_layer_info->opacity_value = opacity_value;
...
GDI_RESULT gdi_layer_set_source_key(BOOL source_key_enable, gdi_color source_key_value)
{
    ...
    #if defined(GDI_USE_RGB565_TO_SHOW_BW)
    gdi_act_layer_info->source_key_enable = KAL_FALSE;
    gdi_act_layer_info->source_key = 0;
    #else
    gdi_act_layer_info->source_key_enable = (kal_bool)source_key_enable;
...
static void gdi_layer_flatten_nb_finish_handler(void* unused_param)
{
    ...
    /*
     * We assume this HISR/DRV callback will not be preempted by VRT or MMI or
    ...
    kal_int32 output_buf_h)
{
    ...
    drv_layer_param.enableSpatialDithering = (layer_struct->cf == GDI_COLOR_FORMAT_UYVY422);
...
static void gdi_layer_govl_nb_finish_handler(void *unused_cbparam)
{
    ...
    gdi_layer_flatten_nb_finish_handler(unused_cbparam);
}
...
/* under construction !*/
    ...
    else
    {
        ...
        flatten_ctrl.flatten_nonblocking_callback = gdi_layer_govl_nb_finish_handler;
        flatten_ctrl.cbparam = NULL;
    }
    ...
    {
        /* Release ownership in gdi_layer_govl_nb_finish_handler() */
    }
                ...
                S32 dest_y2)
{
     ...
     */
    pContextAttrib->enableVsync = KAL_TRUE;
            ...
            gdi_handle handle5)
{
    ...
    GDI_ENTER_CRITICAL_SECTION(gdi_layer_flatten_ext)
    GDI_LCD_LOCK_MUTEX;
    ...
    ret =  gdi_layer_flatten_ext_internal(
     ...
     GDI_LCD_UNLOCK_MUTEX;
     GDI_EXIT_CRITICAL_SECTION(gdi_layer_flatten_ext)
     return ret;
}
            ...
            gdi_handle handle5)
{
    ...
    GDI_ENTER_CRITICAL_SECTION(gdi_layer_flatten_with_clipping_ext)
    GDI_LCD_LOCK_MUTEX;
    ...
    GDI_LCD_UNLOCK_MUTEX;
    GDI_EXIT_CRITICAL_SECTION(gdi_layer_flatten_with_clipping_ext)
    return ret;
}
            ...
            void *param)
{
    ...
    GDI_ENTER_CRITICAL_SECTION(gdi_layer_flatten_nb_ext)
    GDI_LCD_LOCK_MUTEX;

    if (finish_callback == NULL)
    {
        finish_callback = gdi_layer_flatten_nb_dummy_callback;
    }
            ...
            void *param)
{
    ...
    GDI_ENTER_CRITICAL_SECTION(gdi_layer_flatten_nb_with_clip_ext)
    GDI_LCD_LOCK_MUTEX;
                    ...
                    const gdi_callback_struct *nb_finish_callback)
{
   ...
   {
		#ifdef GDI_6_LAYERS
		mr_backstage_widget_store_and_set_blt_layers( &handle0,  &handle1,  &handle2, 
...
/* under construction !*/
        ...
        {
            kal_bool wait_for_composition = !non_blocking;
...
GDI_RESULT gdi_layer_clear(gdi_color bg_color)
{
    ...
    {
        gdi_memset24((U8*) gdi_act_layer->buf_ptr, (U32) bg_color, gdi_act_layer->layer_size);
    }
    #endif
...
void gdi_layer_debug_flatten(void)
{
    ...
    for (i = 0; i < GDI_LAYER_HW_LAYER_COUNT; i++)
    {
        ...
        GDI_ASSERT(GDI_LAYER_GET_FLAG(layer_handle, GDI_LAYER_FLAG_USED));
        if (GDI_LAYER_GET_FLAG(layer_handle, GDI_LAYER_FLAG_HW_UPDATE))
        {
            continue;
        }
        ...
        switch(layer_struct->cf)
        {
            case GDI_COLOR_FORMAT_8:
                ...
                exception_layers[layer_count].mem_pitch = layer_info->column_number; /* width * bpp */
                break;
            case GDI_COLOR_FORMAT_16:
                exception_layers[layer_count].color_format = LCD_EXPT_LAYER_FORMAT_RGB565;
                exception_layers[layer_count].mem_pitch = layer_info->column_number * 2; /* width * bpp */
                break;
            case GDI_COLOR_FORMAT_24:
                exception_layers[layer_count].color_format = LCD_EXPT_LAYER_FORMAT_RGB888;
                exception_layers[layer_count].mem_pitch = layer_info->column_number * 3; /* width * bpp */
                break;
            case GDI_COLOR_FORMAT_32:
                exception_layers[layer_count].color_format = LCD_EXPT_LAYER_FORMAT_ARGB8888;
                exception_layers[layer_count].mem_pitch = layer_info->column_number * 4; /* width * bpp */
                break;
            case GDI_COLOR_FORMAT_32_PARGB:
...
GDI_RESULT gdi_layer_save_bmp_file(gdi_handle layer_handle, CHAR *file_name)
{
    ...
    ret = gdi_image_bmp_encode_file(image_width, image_height, image_depth, image_ptr, file_name);
...
void gdi_layer_resize(S32 width, S32 height)
{
    ...
    /* reset blt region */
    GDI_LAYER_SET_BLT_RECT(gdi_act_layer, 0, 0, width, height);
...
GDI_RESULT gdi_layer_set_opacity_nb_concurrent(gdi_handle layer_handle, BOOL opacity_enable, U8 opacity_value)
{
    ...
    {
        gdi_layer_info[layer->id].opacity_enable = KAL_TRUE;
        #ifdef LCD_MAX_OPACITY
        gdi_layer_info[layer->id].opacity_value = opacity_value * LCD_MAX_OPACITY / 255;  /* shift 3, hw only support 5 bit */
        #else 
        gdi_layer_info[layer->id].opacity_value = opacity_value;
        #endif
        return GDI_SUCCEED;
    }
    ...
    gdi_layer_info[layer->id].opacity_enable = (kal_bool)opacity_enable;
#ifdef LCD_MAX_OPACITY
    gdi_layer_info[layer->id].opacity_value = opacity_value * LCD_MAX_OPACITY / 255;  /* shift 3, hw only support 5 bit */
#else 
    gdi_layer_info[layer->id].opacity_value = opacity_value;
...
GDI_RESULT gdi_layer_resize_nb_concurrent(gdi_handle layer_handle, S32 width, S32 height)
{
    ...
    layer->width = (U16) width;
    layer->height = (U16) height;
    gdi_layer_info[layer->id].column_number = (U16) width;
    gdi_layer_info[layer->id].row_number = (U16) height;
