...
static void update_key_proc_data(key_data_struct *key_data, MMI_BOOL force_update);
...
static MMI_BOOL get_driver_key_data(kbd_data *keydata)
{
	 ...
	 keydata1->Keyevent =  data.rKBDCtrGetData.Keyevent;
...
static void mmi_frm_key_handle_before_poweron(void *paraBuff)
{
    ...
    if (paraBuff != NULL)
    {
        ...
        while ((get_key_evt_func[p->func_type])(&k) == KAL_TRUE);
    }
}
...
void mmi_frm_process_key_in_nvarm(void * para_buff)
{
    ...
    else if (func == mmi_frm_key_handle)
    {
...
static mmi_ret mmi_sys_key_proc(mmi_frm_key_evt_struct *evt)
{
    ...
    if (evt->evt_id == EVT_ID_ON_KEY)
    {
        ...
        #if (!defined (__COSMOS_MMI_PACKAGE__))||defined(__MMI_ENDKEY_BACK_IDLE__)
...
static void mmi_key_handle(mmi_key_evt_struct *mmi_evt_p)
{
    ...
    for (i = 0; i < evt_num; i++)
    {
        if((exec_evt[i].mmi_key_type == KEY_EVENT_DOWN) || (exec_evt[i].mmi_key_type == KEY_HALF_PRESS_DOWN))
        {
...
void mmi_frm_send_key_event(U8 key_code, U8 key_type)
{
    ...
    g_key_cntx.is_key_event = MMI_FALSE;
    update_key_proc_data(&data, MMI_TRUE);
...
#if defined( __MMI_DSM_NEW__)  && defined(__MMI_DSM_NEW_JSKY__)
S32 SKY_get_key_size(void)
{
	return sizeof(g_key_code_map) / sizeof(g_key_code_map[0]);
}
S16 SKY_get_device_key_code(U8 index)
{
	return g_key_code_map[index].device_key_code;
}
S16 SKY_get_mmi_key_code(U8 device_key_code)
{
	return get_mmi_key_code(device_key_code);
}
#endif
 ...
 *  void
 *****************************************************************************/
static void update_key_proc_data(key_data_struct *key_data_p, MMI_BOOL force_update)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((key_data_p->mmi_key_type == KEY_EVENT_DOWN || key_data_p->mmi_key_type == KEY_HALF_PRESS_DOWN) &&
        (g_key_cntx.buf_len == 0) || (force_update == MMI_TRUE))
    {
        memcpy(g_key_cntx.routing_layer[LAYER_INVOKE_SPACE], 
            g_key_cntx.routing_layer[LAYER_SET_SPACE], 
            sizeof(mmi_key_routing_layer_struct) * MMI_KEY_ROUTING_TOTAL);
...
/*****************************************************************************
 * FUNCTION
 *  reset_screen_switch_state
 * DESCRIPTION
 *  This function is used to reset screen switch flag
 *  [UT_function]: reset_screen_switch_state
 * PARAMETERS
 *  key_data_p      : [IN] key data
 * RETURNS
 *  void
 *****************************************************************************/
static void reset_screen_switch_state(key_data_struct *key_data_p)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((key_data_p->mmi_key_type == KEY_EVENT_UP) &&
        (g_key_cntx.buf_len == 0))
    {
        g_key_cntx.is_screen_switch = MMI_FALSE;
    }
...
mmi_ret ut_reset_screen_switch_state(void *para)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
mmi_ret ut_evt_routing_proc_life_check(void *para)
{
    ...
    for (i = 0; i < sizeof(f) / sizeof(f[0]); i++)
    {
        ...
        g_key_cntx.buf_len = 0;
        update_key_proc_data(&key_data, MMI_FALSE);
...
mmi_ret ut_update_key_proc_data(void *para)
{
    ...
    update_key_proc_data(&data, MMI_FALSE);
    //MMI_ASSERT(g_key_cntx.is_screen_switch == MMI_FALSE);
...
const static mmi_unit_test_struct g_key_ut_table[] = 
{
    ...
    {44, ut_reset_screen_switch_state},
    {45, ut_get_exec_key_evt},
    {46, ut_get_mmi_key_events},		
	{47, ut_mmi_frm_set_key_proc},
    {48, ut_evt_routing_full_path_test},
    {49, ut_evt_routing_cat_pre_stop_test},
    {50, ut_evt_routing_list_pre_stop_test},
    {51, ut_evt_routing_on_stop_scr_test},
    {52, ut_evt_routing_on_stop_editor_test}, 
    {53, ut_evt_routing_stop_at_key_handler},
    {54, ut_evt_routing_scr_no_proc_check},
    {55, ut_evt_routing_proc_life_check},
    {56, ut_evt_routing_event_flow_check},
    {57, ut_is_need_write_back},
    {58, ut_mmi_frm_clear_key_events},
    {59, ut_is_combo_key},
    {60, ut_process_key_ptr_change},
    {61, ut_is_power_off_evt},
    {62, ut_is_pre_process_dev_evt}
};
