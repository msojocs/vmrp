...
static MHdl *PCM_Strm_Open_Adapter(void(*handler)( MHdl *handle, Media_Event event ), STFSAL *pstFSAL, void *param)
{
	return PCM_Strm_Open(handler, param);
}
#endif
void bitstream_audio_open_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    #endif
            vpFormat.mediaType = msg_p->codec_type;
	...
	case MED_TYPE_PCM_8K:
		pcm_param.isStereo = KAL_FALSE;
		pcm_param.bitPerSample = 16;
		pcm_param.sampleFreq = 8000;
		pcm_param.forceVoice = KAL_FALSE;
		
		openFunc = PCM_Strm_Open_Adapter;
		param = &pcm_param;
		break;
#endif
