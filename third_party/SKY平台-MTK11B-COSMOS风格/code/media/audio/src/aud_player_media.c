...
static void _aud_player_media_event_callback_hdlr(void* data_p, kal_uint16 date_len)
{
    ...
    {
        _AUD_PLAYER_MEDIA_TRACE(self_p->state, self_p->mhdl_handle_p, self_p->time_stamp);
        return;
    }
    
    /* Process media data */
    event = mhdl_handle_p->Process(mhdl_handle_p, (Media_Event) media_event_p->event);
    _AUD_PLAYER_MEDIA_TRACE(event, media_event_p->event, mhdl_handle_p);
...
static kal_int32 _aud_player_media_close(med_aud_player_t* player_p)
{
    ...
    if (self_p->mhdl_handle_p)
    {
        ...
        _AUD_PLAYER_MEDIA_TRACE(drv_result, self_p->mhdl_handle_p, -1);             
...
static kal_int32 _aud_player_media_play(med_aud_player_t* player_p)
{
	...
	if (dsmLoopPCM > 0)
	{
		aud_util_alloc_ring_buffer_ext(1024*2, &self_p->ring_buffer_p);
		self_p->mhdl_handle_p->SetBuffer(self_p->mhdl_handle_p, (kal_uint8*)self_p->ring_buffer_p, 1024*2);		
	}
	else
	{
                                       ...
                                       kal_uint32           data_len)
{
    ...
    _AUD_PLAYER_MEDIA_TRACE(set_type, (kal_uint32)data_p, data_len);
    ...
    switch (set_type)
    {
        ...
        {
            self_p->mhdl_handle_p->SetBuffer(self_p->mhdl_handle_p, (kal_uint8*)data_p, (kal_uint32)data_len);
            break;
        }
                                       ...
                                       kal_uint32           data_len)
{
    ...
    switch (get_type)
    {
        ...
        case MEDIA_PLAYER_GET_BUILD_CACHE_PROGRESS:
        {
            if (data_len >= sizeof(kal_uint32))
            {
                self_p->mhdl_handle_p->BuildCache(self_p->mhdl_handle_p, &drv_result, (kal_uint32*)data_p);            
...
med_aud_player_t* aud_player_media_construct(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    self_p = (med_aud_player_media_t*) med_alloc_aud_mem(sizeof(med_aud_player_media_t));
