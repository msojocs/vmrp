...
/* ---- SKY_MULTIMEDIA start ---- */
	...
	if(!mr_media_audio_is_recording())
...
static kal_uint8 _aud_recorder_media_update_header(med_aud_recorder_t* recorder_p)
{
        ...
        self_p->format == MEDIA_WAV_DVI_ADPCM)
    {
        ...
        _AUD_RECORDER_MEDIA_TRACE(result, self_p->file_handle, len);
...
static void _aud_recorder_media_terminate(med_aud_recorder_t* recorder_p, med_aud_recorder_reason_t term_reason)
{
    ...
    _AUD_RECORDER_MEDIA_TRACE(term_reason, recorder_p, self_p->file_offset);
...
/* ---- SKY_MULTIMEDIA start ---- */
	...
	if(mr_media_audio_is_recording())
	{
		mr_media_record_data_ind(buf_p,buf_len * 2);
		fs_result = FS_NO_ERROR;
		len = buf_len * 2;
		//kal_prompt_trace(MOD_MED, "media task mpchat  Aud buff address= %d  size=%d", aud_context_p->buf_p,aud_context_p->buf_len * 2);
	}
	else
#endif /* __MMI_DSM_MEDIA_EXT__ */ 
/* ---- SKY_MULTIMEDIA end ---- */
            ...
            *written_len = len;
            ...
            _AUD_RECORDER_MEDIA_TRACE(self_p->file_handle, self_p->file_offset, self_p->size_committed);
...
static void _aud_recorder_media_time_limit_hdlr(void* data_p, kal_uint16 date_len)
{
        ...
        (self_p->state == AUD_MEDIA_RECORD))
    {
        for (write_loop = 0; write_loop < 2; write_loop ++) 
        {
            ...
            _AUD_RECORDER_MEDIA_TRACE(result, written_len, self_p->file_handle);
...
static void _aud_recorder_media_read_data_fct(void* data_p, kal_uint16 date_len)
{
        ...
        (self_p->state == AUD_MEDIA_RECORD))
    {
        ...
        /* It's fine to encounter ring buffer boundary here. When stop record, we will
         * write data buffer to file system twice. Hence, we don't need to read buffer
         * one more time here.
         */
...
static void _aud_recorder_media_event_callback_hdlr(void* data_p, kal_uint16 date_len)
{
    ...
    _AUD_RECORDER_MEDIA_TRACE(media_event_p->event, media_event_p->recorder_p, self_p->state);
...
static void _aud_recorder_media_event_callback_fct(Media_Event event)
{
    ...
    _AUD_RECORDER_MEDIA_TRACE(event, g_recorder_p, -1);
...
static kal_int16 _aud_recorder_media_get_record_format(kal_wchar* file_name, kal_uint8 input_format)
{
    ...
    switch (media_type)
    {
        ...
        case MED_TYPE_AAC:           record_format = MEDIA_AAC;           break;
    ...
    if (record_format == MEDIA_FORMAT_NONE)
    {
        ...
        if (input_format != MEDIA_FORMAT_NONE)
        {
...
static kal_int32 _aud_recorder_media_open(med_aud_recorder_t* recorder_p, med_aud_recorder_cfg_t* cfg_p)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MED_ASSERT((recorder_p != NULL) && (cfg_p->file_name != NULL));
    self_p = PTHIS(recorder_p, med_aud_recorder_media_t, itf);
...
/* ---- SKY_MULTIMEDIA end ---- */
    ...
    if (self_p->file_handle >= 0) /* File already exists */
    {
        ...
        _AUD_RECORDER_MEDIA_TRACE(self_p->file_handle, is_file_empty, len);
    }
    else /* File does not exist */
    {
        ...
        _AUD_RECORDER_MEDIA_TRACE(self_p->file_handle, is_file_empty, -1);
    }
...
/* ---- SKY_MULTIMEDIA end ---- */
    ...
    if (self_p->file_handle >= 0)
    {
        ...
        _AUD_RECORDER_MEDIA_TRACE(self_p->format, cfg_p->format, self_p->file_handle);
...
/* ---- SKY_MULTIMEDIA end ---- */
    ...
    _AUD_RECORDER_MEDIA_TRACE(recorder_p, self_p->format, self_p->file_handle);
...
static kal_int32 _aud_recorder_media_record(med_aud_recorder_t* recorder_p)
{
    ...
    _AUD_RECORDER_MEDIA_TRACE(recorder_p, self_p->state, self_p->format);
    ...
    param_p = (void*) aud_rec_get_record_quality_param(self_p->format,self_p->quality);

    _AUD_RECORDER_MEDIA_TRACE(self_p->format, media_format, param_p);
    
    /*------------------------- Check if it is appendable ------------------*/
/* ---- SKY_MULTIMEDIA start ---- */
/*MTK 1212版本之前不需要移植*/
#if defined(__MMI_DSM_NEW__) && defined(__DSM_MEDIA_EXT_AUD_REC__)&& defined(__MTK_TARGET__) &&  (__MR_CFG_VAR_MTK_VERSION__ >= 0x11B1212)
     if (self_p->format == MEDIA_AMR)
    {
        media_format = MEDIA_FORMAT_AMR;

        if (self_p->quality == AUD_REC_QUALITY_AUTO)
        {
            param_p = (void*)0x03; /* Best quality */
        }
        /* 10: 4.75kbps ~ 3: 12.2kbps */
        else if (self_p->quality >= AUD_REC_QUALITY_AMR_4_75 && 
                 self_p->quality <= AUD_REC_QUALITY_AMR_12_2)
        {
            param_p = (void*)(10 - (self_p->quality - AUD_REC_QUALITY_AMR_4_75));
        }
        else
        {
            if (self_p->quality != AUD_REC_QUALITY_LOW)
            {
                param_p = (void*)0x03;
            }
...
/* ---- SKY_MULTIMEDIA end ---- */
    if ((FS_GetFileSize(self_p->file_handle, &len) == FS_NO_ERROR) && (len > 0))
    {
        ...
        _AUD_RECORDER_MEDIA_TRACE(result, self_p->file_handle, -1);
    ...
    _AUD_RECORDER_MEDIA_TRACE(result, media_format, param_p);
...
static kal_int32 _aud_recorder_media_pause(med_aud_recorder_t* recorder_p)
{
    ...
    _AUD_RECORDER_MEDIA_TRACE(drv_result, recorder_p, -1);

    /* Write remaining data to file */
    result = _aud_recorder_media_write_to_file(recorder_p, &written_len);
    _AUD_RECORDER_MEDIA_TRACE(result, written_len, -1);
                                         ...
                                         kal_uint32             data_len)
{
    ...
    _AUD_RECORDER_MEDIA_TRACE(set_type, (kal_uint32)data_p, data_len);
