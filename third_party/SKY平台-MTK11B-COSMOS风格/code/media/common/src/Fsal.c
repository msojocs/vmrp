...
#elif FSAL_PLATFORM_KAL
   {
      ...
      if ((pstFSAL->hFile = DRM_open_file((kal_wchar*)szFile, uFlags, (kal_uint8)pstFSAL->uDRMpermission)) < 0) {
         ...
         if (pstFSAL->iFSErrorCode==FS_DEVICE_BUSY || pstFSAL->iFSErrorCode==FS_LOCK_MUTEX_FAIL)
            ...
            kal_trace(TRACE_ERROR, FSAL_TRACE_OPEN_ERROR, pstFSAL->iFSErrorCode);
            return FSAL_OPEN_ERROR;
         }
      }
   }
...
#elif FSAL_PLATFORM_KAL
   {
      ...
      if ((pstFSAL->iFSErrorCode = DRM_close_file(pstFSAL->hFile)) < 0) {
         if (pstFSAL->iFSErrorCode==FS_DEVICE_BUSY || pstFSAL->iFSErrorCode==FS_LOCK_MUTEX_FAIL)
            ...
            kal_trace(TRACE_ERROR, FSAL_TRACE_CLOSE_ERROR, pstFSAL->iFSErrorCode);
            return FSAL_CLOSE_ERROR;
         }
      }
      pstFSAL->hFile = -1;
   ...
   FSAL_CHECK_ARG(pstFSAL->hFile>=0);
   {
      ...
      if (pstFSAL->iFSErrorCode < 0) {
         if (pstFSAL->iFSErrorCode == FS_DEVICE_BUSY || pstFSAL->iFSErrorCode == FS_LOCK_MUTEX_FAIL)
            ...
            kal_trace(TRACE_ERROR, FSAL_TRACE_READ_ERROR, pstFSAL->iFSErrorCode);
            return FSAL_READ_ERROR;
         }
      }
      if (pstFSAL->uLastReadCount != uSize) {
   ...
   FSAL_CHECK_ARG(pstFSAL->hFile>=0);
   {
      ...
      if ((pstFSAL->iFSErrorCode<0) || (uBytesWritten!=uSize)) {
         if(pstFSAL->iFSErrorCode==FS_DEVICE_BUSY || pstFSAL->iFSErrorCode==FS_LOCK_MUTEX_FAIL)
            ...
            kal_trace(TRACE_ERROR, FSAL_TRACE_WRITE_ERROR, pstFSAL->iFSErrorCode);
            return FSAL_WRITE_ERROR;
         }
      }
   }
...
FSAL_Status FSAL_Direct_Seek(STFSAL *pstFSAL, kal_uint32 uOffset)
{
...
#elif FSAL_PLATFORM_KAL
   {
      FSAL_CHECK_ARG(pstFSAL->hFile>=0);
      {
         ...
         {
            kal_int64 iResult;
            pstFSAL->iFSErrorCode = iResult = DRM_seek_large_file(pstFSAL->hFile, uOffset, FS_FILE_BEGIN);
            if ((iResult < 0) || (iResult != (kal_int64) uOffset)) {
               if (pstFSAL->iFSErrorCode==FS_DEVICE_BUSY || pstFSAL->iFSErrorCode==FS_LOCK_MUTEX_FAIL)
                  ...
                  kal_trace(TRACE_ERROR, FSAL_TRACE_SEEK_ERROR, pstFSAL->iFSErrorCode);
                  return FSAL_SEEK_ERROR;
               }
            }
         }
...
#ifndef __LOW_COST_SUPPORT_ULC__
   {
      ...
      if (pstFSAL->iFSErrorCode < 0) {
         if (pstFSAL->iFSErrorCode == FS_DEVICE_BUSY || pstFSAL->iFSErrorCode == FS_LOCK_MUTEX_FAIL)
            ...
            kal_trace(TRACE_ERROR, FSAL_TRACE_GET_FILE_POSITION_ERROR, pstFSAL->iFSErrorCode);
      ...
      if (pstFSAL->iFSErrorCode < 0) {
         if (pstFSAL->iFSErrorCode == FS_DEVICE_BUSY || pstFSAL->iFSErrorCode == FS_LOCK_MUTEX_FAIL)
            return FSAL_DEVICE_BUSY;
         	...
         	kal_trace(TRACE_ERROR, FSAL_TRACE_GET_FILE_SIZE_ERROR, pstFSAL->iFSErrorCode);
            return FSAL_FATAL_ERROR;
         }
      }
   }
