 ...
 * DESCRIPTION
 *  This function is to called when DC calibration is done.
 * PARAMETERS
...
{  
	aud_send_msg_to_nvram(MSG_ID_NVRAM_WRITE_REQ, NVRAM_EF_AUDIO_DC_CALIBRATION_LID, pdc_offset, NVRAM_EF_AUDIO_DC_CALIBRATION_SIZE);
}
    ...
    switch (parm_ptr->file_idx)
    {
        ...
        case NVRAM_EF_AUDIO_PARAM_LID:
            ...
            if (med_context_p->nvram_state == MED_POWERON_READING_NVRAM)
            {
                ...
                Media_SetAudioCompensation(MEDIA_AUD_COMP_LOUDSPEAKER, nvram_param_p->audio_compensation_coeff[0]);
                Media_SetAudioCompensation(MEDIA_AUD_COMP_EARPHONE, nvram_param_p->audio_compensation_coeff[1]);
                Media_SetAudioCompensation(MEDIA_AUD_COMP_LOUDSPEAKER_RINGTONE, nvram_param_p->audio_compensation_coeff[2]);
...
#if (defined(__MMI_DSM_NEW__)||defined(__DSM_SPLAYER__))&& defined(__MTK_TARGET__) && defined(MT6255)
	if ((ilm_ptr->msg_id >= MSG_ID_SPLAYER_AUDIO_BEGIN)&&(ilm_ptr->msg_id <= MSG_ID_SPLAYER_AUDIO_END))
	{
		extern void splayer_aud_msg_req_hdlr(ilm_struct *ilm_ptr);
		kal_prompt_trace(MOD_MED,"splayer_aud_msg_req_hdlr");
		splayer_aud_msg_req_hdlr(ilm_ptr);
	}
	else
#endif
#if defined(__MMI_DSM_NEW__) && defined(__DSM_MEDIA_EXT_H264_ENCODE__)&& defined(__MTK_TARGET__)
       if (ilm_ptr->msg_id == MSG_ID_MEDIA_CAM_MPCHAT_DATAREQ_IND)
    	{
             mr_media_cam_datareq_ind_hdlr(ilm_ptr);
    	}
	else
#endif
#if defined(__MMI_DSM_NEW__) && defined(__DSM_MEDIA_EXT_H264_ENCODE__)&& defined(__MTK_TARGET__)
	if (ilm_ptr->msg_id == MSG_ID_MPCHAT_ENCODE_REQ)
	{
		extern void mr_media_Encode_req_hdlr(void* pMsg);
		mr_media_Encode_req_hdlr((void *)ilm_ptr->local_para_ptr);	
	}
	else
