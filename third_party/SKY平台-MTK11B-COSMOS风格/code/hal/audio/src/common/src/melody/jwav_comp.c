...
#define JWAV_HISR_TRACE   kal_dev_trace
#define JWAV_TASK_TRACE   kal_trace
...
Media_Handle  *JWav_Open( void (*handler)( Media_Handle *handle, Media_Event event ), Media_Wav_Param **param )
{
   WAV_Player *player;
   Media_Handle *handle;
#if defined(MELODY_FORCE_ONEPLAYER)
   if(!MelodyComp_QUERYNUM())
      return 0;
   MelodyComp_INCREASE(1);
#endif
   ...
   *param = &(player->Wav_Param);
   ...
   JWAV_TASK_TRACE( TRACE_GROUP_JWAV, MIDI_JWAV_OPEN, handle );
...
Media_Status  JWav_Close( Media_Handle *handle )
{
   ...
   JWAV_TASK_TRACE( TRACE_GROUP_JWAV, MIDI_JWAV_CLOSE, handle );
#if defined(MELODY_FORCE_ONEPLAYER)
   MelodyComp_DECREASE();
#endif
