...
- `VMRP_E2E_KEEP_TMP=1 pnpm vitest run test/e2e/wbrw/proxy.test.ts --reporter=verbose` passed after the URL reconstruction change. Latest artifact: `/tmp/vmrp-e2e-x2ZvJz`; workspace: `/tmp/vmrp-ws-HQZXfE`.
- Cache evidence: `/tmp/vmrp-ws-HQZXfE/mythroad/brw/http/cache3/14056.dat` is 477 bytes and contains `<title>百度一下</title>`, `http://wap.baidu.com/`, and multiple `百度` strings.
- PPM evidence: `/tmp/vmrp-e2e-x2ZvJz/loaded.ppm` is 240x320 with 149 unique colors; compared with `home.ppm`, top 90px diff is 5320 pixels and full-screen diff is 5491 pixels. URL bar samples are `(41,10)=[248,240,168]`, `(100,10)=[96,100,96]`, and `(66,10)=[96,100,96]`.
...
PPM 验证 (loaded.ppm, /tmp/vmrp-e2e-atstsl): 黄色标题栏"第1页 - WAP下载站"、URL 链接行、
正文 软件列表/mynes/RX管理器/vmrp安装包… 全部来自目标站,无表单、无模板残留;
...
1. **E2E 注入按键全部变长按**: 测试为剪贴板粘贴稳定性设 `VMRP_E2E_HOLD_MS=1500`,vmrp 会把
   ...
   非场景代码),`vmrp-e2e.ts key(name, timeoutMs, holdMs)`。实测: DOWN 用 120ms 单步移动;
...
截图证据(/tmp/vmrp-e2e-WjYiU6/sublink.png): 标题栏 `S地牢-斗圣 - WAP下载站`,正文
...
- 最终用例序列(回车直接导航): 短按 DOWN×2(120ms,每次一行) → 焦点框画在 RX管理器
  ...
  但非必需。
- 通过轮 /tmp/vmrp-e2e-bDO9Yu: 第三个 /page2 发出(tag1=/mrp/sky_SaiNes-v1010,
...
1. **页面数据无罪**(已证): 解码器(/tmp/skydec/decode.js,按 wbrw-sky-format.md 逐记录解码)
   ...
   - 推论A: 7715.dat/26660.dat 是**本地 vmrp 运行产物**,不是真机数据(否则不可能与
...
3. 反汇编任务已派发(game.ext 焦点扫描 0x275B4/0x28718、paint 0x27F50、ENTER 激活路径),
   等待结论后决定修复层(生成器记录结构 vs vmrp C 侧)。
