...
describe("sanguo 启动", () => {
  ...
  it("游戏正常启动并处理确认键", async () => {
    ...
    vmrp = await VmrpE2e.start("test/fixtures/sanguo_490111.mrp", { workDir: ws.dir });
    ...
    const dialog = await vmrp.waitForPixel(120, 8, [48, 180, 232], {
      name: "dialog",
      timeoutMs: 180_000,
      intervalMs: 2_000
    });
    ...
    const splash = await vmrp.screenDraw(1, "splash");
    // 标题"幻"字金黄描边
    ...
    // 服务器停运后这是该画面唯一有效的交互。
    await vmrp.key("ENTER", 8_000);
    expect(await vmrp.waitForExit(8_000)).toBe(true);
  }, 240_000);
});
